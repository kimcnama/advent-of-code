// CkPfxU.h: interface for the CkPfxU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkPfxU
class CK_VISIBLE_PUBLIC CkPfxU  : public CkUtf16Base
{
	...
	// used when writing, or the algorithms used by the loaded PFX.
	// 
	const uint16_t *algorithmId(void);
	// The encryption algorithm to be used when writing a PFX. After loading a PFX,
	// this property is set to the encryption algorithm used by the loaded PFX. (This
	// is the algorithm used for the "shrouded key bag", which is internal to the PFX.)
	// 
	...
	// used when writing, or the algorithms used by the loaded PFX.
	// 
	void put_AlgorithmId(const uint16_t *newVal);
	...
	// by this property is only used when the Algorithmid = "pbes2".
	// 
	const uint16_t *pbes2CryptAlg(void);
	// If the AlgorithmId property equals "pbes2", then this is the encryption
	// algorithm to be used when writing the PFX, or used by the PFX that was loaded.
	// If the AlgorithmId is not equal to "pbes2", then the value of this property is
	// meaningless.
	...
	// by this property is only used when the Algorithmid = "pbes2".
	// 
	void put_Pbes2CryptAlg(const uint16_t *newVal);
	...
	// specified by this property is only used when the Algorithmid = "pbes2".
	// 
	const uint16_t *pbes2HmacAlg(void);
	// If the AlgorithmId property equals "pbes2", then this is the HMAC hash algorithm
	// to be used when writing the PFX, or used by the PFX that was loaded. If the
	// AlgorithmId is not equal to "pbes2", then the value of this property is
	// meaningless.
	...
	// specified by this property is only used when the Algorithmid = "pbes2".
	// 
	void put_Pbes2HmacAlg(const uint16_t *newVal);
	...
	bool AddCert(CkCertU &cert, bool includeChain);
	...
	bool AddPrivateKey(CkPrivateKeyU &privKey, CkCertChainU &certChain);
	...
	CkCertU *FindCertByLocalKeyId(const uint16_t *localKeyId, const uint16_t *encoding);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkPrivateKeyU *GetPrivateKey(int index);
	...
	// PFX.
	// 
	bool GetSafeBagAttr(bool forPrivateKey, int index, const uint16_t *attrName, CkString &outStr);
	// Can be called to get one of the following safebag attributes for the Nth private
	// key or certificate in the PFX. forPrivateKey should be true for a private key, and
	// false for a certificate. The index is the index of the certificate or key in
	// the PFX. The attrName can be one of the following:
	...
	// PFX.
	// 
	const uint16_t *getSafeBagAttr(bool forPrivateKey, int index, const uint16_t *attrName);
	// Can be called to get one of the following safebag attributes for the Nth private
	// key or certificate in the PFX. forPrivateKey should be true for a private key, and
	// false for a certificate. The index is the index of the certificate or key in
	// the PFX. The attrName can be one of the following:
	...
	// PFX.
	// 
	const uint16_t *safeBagAttr(bool forPrivateKey, int index, const uint16_t *attrName);
	...
	// 
	bool ImportToWindows(bool exportable, bool userProtected, bool machineKeyset, bool allowOverwriteKey, bool allowExport, const uint16_t *leafStore, const uint16_t *intermediateStore, const uint16_t *rootStore, const uint16_t *extraOptions);
#endif
	...
	// ...
	// -----END CERTIFICATE----- 
	bool LoadPem(const uint16_t *pemStr, const uint16_t *password);
	...
	// 
	bool LoadPfxBytes(CkByteData &pfxData, const uint16_t *password);
	...
	// method (below) for details.
	// 
	bool LoadPfxEncoded(const uint16_t *encodedData, const uint16_t *encoding, const uint16_t *password);
	...
	//      }
	// 
	bool LoadPfxFile(const uint16_t *path, const uint16_t *password);
	...
	// 
	bool SetSafeBagAttr(bool forPrivateKey, int index, const uint16_t *name, const uint16_t *value, const uint16_t *encoding);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkJavaKeyStoreU *ToJavaKeyStore(const uint16_t *alias, const uint16_t *password);
	...
	// ...
	// -----END CERTIFICATE----- 
	bool ToPemEx(bool extendedAttrs, bool noKeys, bool noCerts, bool noCaCerts, const uint16_t *encryptAlg, const uint16_t *password, CkString &outStr);
	// Write the PFX to a PEM formatted string. If extendedAttrs is true, then extended
	// properties (Bag Attributes and Key Attributes) are output. If noKeys is true,
	// then no private keys are output. If noCerts is true, then no certificates are
	// output. If noCaCerts is true, then no CA certs or intermediate CA certs are output.
	...
	// ...
	// -----END CERTIFICATE----- 
	const uint16_t *toPemEx(bool extendedAttrs, bool noKeys, bool noCerts, bool noCaCerts, const uint16_t *encryptAlg, const uint16_t *password);
