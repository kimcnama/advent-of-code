...
class CK_VISIBLE_PUBLIC CkHttpU  : public CkClassWithCallbacksU
{
	...
	// property should be set to a JSON string containing the information needed to
	// compute and automatically add the HTTP Signature. See the example(s) below.
	const uint16_t *authSignature(void);
	// If set, then automatically adds the Authorization: Signature keyId="...",
	// algorithm="...", headers="...", signature="..." header to all requests. This
	// property should be set to a JSON string containing the information needed to
	// compute and automatically add the HTTP Signature. See the example(s) below.
	void put_AuthSignature(const uint16_t *newVal);
	...
	// 
	bool CreateTimestampRequest(const uint16_t *hashAlg, const uint16_t *hashVal, const uint16_t *reqPolicyOid, bool addNonce, bool reqTsaCert, CkBinDataU &timestampToken);
	...
	//     1: Revoked
	//     2: Unknown
	int OcspCheck(const uint16_t *domain, int port);

	// Creates an asynchronous task to call the OcspCheck method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *OcspCheckAsync(const uint16_t *domain, int port);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkHttpResponseU *PFile(const uint16_t *verb, const uint16_t *url, const uint16_t *localFilePath, const uint16_t *contentType, bool md5, bool gzip);

	// Creates an asynchronous task to call the PFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *PFileAsync(const uint16_t *verb, const uint16_t *url, const uint16_t *localFilePath, const uint16_t *contentType, bool md5, bool gzip);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkHttpResponseU *QuickRequestParams(const uint16_t *verb, const uint16_t *url, CkJsonObjectU &json);

	// Creates an asynchronous task to call the QuickRequestParams method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *QuickRequestParamsAsync(const uint16_t *verb, const uint16_t *url, CkJsonObjectU &json);
	...
	// 
	bool S3_GenPresignedUrl(const uint16_t *httpVerb, bool useHttps, const uint16_t *bucketName, const uint16_t *path, int numSecondsValid, const uint16_t *awsService, CkString &outStr);
	// Generates a temporary pre-signed URL for Amazon S3 using AWS Signature V4.
	// Requires that the AwsSecretKey, AwsAccessKey, and AwsRegion properties be set to
	// valid values prior to calling this method. Also requires the AwsEndpoint
	// property to be set if the endpoint is different than "s3.amazonaws.com".
	...
	// The httpVerb is the HTTP verb such as "GET", "PUT", "POST", "DELETE", etc.  The awsService is a string naming the AWS service, such as "s3" or "s3-accelerate".   If useHttps is true, then the URL begins with "https://", otherwise it begins with "http://".
	// 
	const uint16_t *s3_GenPresignedUrl(const uint16_t *httpVerb, bool useHttps, const uint16_t *bucketName, const uint16_t *path, int numSecondsValid, const uint16_t *awsService);
	...
	bool SetAuthPrivateKey(const uint16_t *publicKeyId, CkPrivateKeyU &privKey);
