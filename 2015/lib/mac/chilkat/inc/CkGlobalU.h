// CkGlobalU.h: interface for the CkGlobalU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkGlobalU
class CK_VISIBLE_PUBLIC CkGlobalU  : public CkUtf16Base
{
	...
	CkGlobalU(const CkGlobalU &);
	CkGlobalU &operator=(const CkGlobalU &);

    public:
	CkGlobalU(void);
	virtual ~CkGlobalU(void);

	

	static CkGlobalU *createNew(void);
	...
	// If set, indicates the path of a log file to be used by the thread pool thread
	// and each of the pool worker threads for logging async activity.
	const uint16_t *threadPoolLogPath(void);
	// If set, indicates the path of a log file to be used by the thread pool thread
	// and each of the pool worker threads for logging async activity.
	void put_ThreadPoolLogPath(const uint16_t *newVal);
	...
	bool ThreadPoolLogLine(const uint16_t *str);
	...
	// 
	bool UnlockBundle(const uint16_t *bundleUnlockCode);
