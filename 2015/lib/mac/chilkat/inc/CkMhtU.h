// CkMhtU.h: interface for the CkMhtU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkMhtU
class CK_VISIBLE_PUBLIC CkMhtU  : public CkClassWithCallbacksU
{
	public:
	bool m_cbOwned;
	...
	static CkMhtU *createNew(void);
	

	CkMhtU(bool bCallbackOwned);
	static CkMhtU *createNew(bool bCallbackOwned);
	...
	// When processing an HTML file or string (not a website URL), this defines the
	// base URL to be used when converting relative HREFs to absolute HREFs.
	const uint16_t *baseUrl(void);
	// When processing an HTML file or string (not a website URL), this defines the
	// base URL to be used when converting relative HREFs to absolute HREFs.
	void put_BaseUrl(const uint16_t *newVal);
	...
	// If problems are experienced, the before/after HTML can be analyzed to help
	// determine the cause.
	const uint16_t *debugHtmlAfter(void);
	// A filename to save the result HTML when converting a URL, file, or HTML string.
	// If problems are experienced, the before/after HTML can be analyzed to help
	// determine the cause.
	void put_DebugHtmlAfter(const uint16_t *newVal);
	...
	// If problems are experienced, the before/after HTML can be analyzed to help
	// determine the cause.
	const uint16_t *debugHtmlBefore(void);
	// A filename to save the input HTML when converting a URL, file, or HTML string.
	// If problems are experienced, the before/after HTML can be analyzed to help
	// determine the cause.
	void put_DebugHtmlBefore(const uint16_t *newVal);
	...
	// proxy string should be formatted as "hostname:port", such as
	// "www.chilkatsoft.com:100".
	const uint16_t *proxy(void);
	// (Optional) A proxy host:port if a proxy is necessary to access the Internet. The
	// proxy string should be formatted as "hostname:port", such as
	// "www.chilkatsoft.com:100".
	void put_Proxy(const uint16_t *newVal);
	...
	// (Optional) Specifies the login if a a Web page is accessed that requires a login
	const uint16_t *webSiteLogin(void);
	// (Optional) Specifies the login if a a Web page is accessed that requires a login
	void put_WebSiteLogin(const uint16_t *newVal);
	...
	// The optional domain name to be used with NTLM / Kerberos / Negotiate
	// authentication.
	const uint16_t *webSiteLoginDomain(void);
	// The optional domain name to be used with NTLM / Kerberos / Negotiate
	// authentication.
	void put_WebSiteLoginDomain(const uint16_t *newVal);
	...
	// Optional) Specifies the password if a a Web page is accessed that requires a
	// login and password
	const uint16_t *webSitePassword(void);
	// Optional) Specifies the password if a a Web page is accessed that requires a
	// login and password
	void put_WebSitePassword(const uint16_t *newVal);
	...
	void AddCustomHeader(const uint16_t *name, const uint16_t *value);
	...
	void AddExternalStyleSheet(const uint16_t *url);
	...
	void ExcludeImagesMatching(const uint16_t *pattern);
	...
	bool GetAndSaveEML(const uint16_t *url_or_htmlFilepath, const uint16_t *emlPath);

	// Creates an asynchronous task to call the GetAndSaveEML method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetAndSaveEMLAsync(const uint16_t *url_or_htmlFilepath, const uint16_t *emlPath);
	...
	bool GetAndSaveMHT(const uint16_t *url_or_htmlFilepath, const uint16_t *mhtPath);

	// Creates an asynchronous task to call the GetAndSaveMHT method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetAndSaveMHTAsync(const uint16_t *url_or_htmlFilepath, const uint16_t *mhtPath);
	...
	bool GetAndZipEML(const uint16_t *url_or_htmlFilepath, const uint16_t *zipEntryFilename, const uint16_t *zipFilename);

	// Creates an asynchronous task to call the GetAndZipEML method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetAndZipEMLAsync(const uint16_t *url_or_htmlFilepath, const uint16_t *zipEntryFilename, const uint16_t *zipFilename);

	// Creates an MHT file from a web page or HTML file, compresses, and appends to a
	// new or existing Zip file. All external images and style sheets are downloaded
	// and embedded in the MHT.
	bool GetAndZipMHT(const uint16_t *url_or_htmlFilepath, const uint16_t *zipEntryFilename, const uint16_t *zipFilename);

	// Creates an asynchronous task to call the GetAndZipMHT method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetAndZipMHTAsync(const uint16_t *url_or_htmlFilepath, const uint16_t *zipEntryFilename, const uint16_t *zipFilename);
	...
	// data as a string.
	bool GetEML(const uint16_t *url_or_htmlFilepath, CkString &outStr);
	// Creates EML from a web page or HTML file, and returns the EML (MIME) message
	// data as a string.
	const uint16_t *getEML(const uint16_t *url_or_htmlFilepath);
	// Creates EML from a web page or HTML file, and returns the EML (MIME) message
	// data as a string.
	const uint16_t *eML(const uint16_t *url_or_htmlFilepath);

	// Creates an asynchronous task to call the GetEML method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetEMLAsync(const uint16_t *url_or_htmlFilepath);

	// Creates MHT from a web page or local HTML file, and returns the MHT (MIME)
	// message data as a string
	bool GetMHT(const uint16_t *url_or_htmlFilepath, CkString &outStr);
	// Creates MHT from a web page or local HTML file, and returns the MHT (MIME)
	// message data as a string
	const uint16_t *getMHT(const uint16_t *url_or_htmlFilepath);
	// Creates MHT from a web page or local HTML file, and returns the MHT (MIME)
	// message data as a string
	const uint16_t *mHT(const uint16_t *url_or_htmlFilepath);

	// Creates an asynchronous task to call the GetMHT method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetMHTAsync(const uint16_t *url_or_htmlFilepath);
	...
	// returned.
	bool HtmlToEML(const uint16_t *htmlText, CkString &outStr);
	// Creates an in-memory EML string from an in-memory HTML string. All external
	// images and style sheets are downloaded and embedded in the EML string that is
	// returned.
	const uint16_t *htmlToEML(const uint16_t *htmlText);

	// Creates an asynchronous task to call the HtmlToEML method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *HtmlToEMLAsync(const uint16_t *htmlText);

	// Creates an EML file from an in-memory HTML string. All external images and style
	// sheets are downloaded and embedded in the EML file.
	bool HtmlToEMLFile(const uint16_t *html, const uint16_t *emlFilename);

	// Creates an asynchronous task to call the HtmlToEMLFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *HtmlToEMLFileAsync(const uint16_t *html, const uint16_t *emlFilename);
	...
	bool HtmlToMHT(const uint16_t *htmlText, CkString &outStr);
	// Creates an in-memory MHT web archive from an in-memory HTML string. All external
	// images and style sheets are downloaded and embedded in the MHT string.
	const uint16_t *htmlToMHT(const uint16_t *htmlText);

	// Creates an asynchronous task to call the HtmlToMHT method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *HtmlToMHTAsync(const uint16_t *htmlText);
	...
	bool HtmlToMHTFile(const uint16_t *html, const uint16_t *mhtFilename);

	// Creates an asynchronous task to call the HtmlToMHTFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *HtmlToMHTFileAsync(const uint16_t *html, const uint16_t *mhtFilename);
	...
	void RemoveCustomHeader(const uint16_t *name);
	...
	bool UnpackMHT(const uint16_t *mhtFilename, const uint16_t *unpackDir, const uint16_t *htmlFilename, const uint16_t *partsSubDir);

	// Same as UnpackMHT, except the MHT is passed in as an in-memory string.
	bool UnpackMHTString(const uint16_t *mhtString, const uint16_t *unpackDir, const uint16_t *htmlFilename, const uint16_t *partsSubDir);
