// CkXmlDSigU.h: interface for the CkXmlDSigU class.
//
//////////////////////////////////////////////////////////////////////

// This header is generated for Chilkat 9.5.0.89

#ifndef _CkXmlDSigU_H
#define _CkXmlDSigU_H
...
// CLASS: CkXmlDSigU
class CK_VISIBLE_PUBLIC CkXmlDSigU  : public CkUtf16Base
{
	...
	CkXmlDSigU(const CkXmlDSigU &);
	CkXmlDSigU &operator=(const CkXmlDSigU &);

    public:
	CkXmlDSigU(void);
	virtual ~CkXmlDSigU(void);

	

	static CkXmlDSigU *createNew(void);
	...
	// has the same filename as the file in the local filesystem.
	// 
	const uint16_t *externalRefDirs(void);
	// May contain a set of directory paths specifying where referenced external files
	// are located. Directory paths should be separated using a semicolon character.
	// The default value of this property is the empty string which means no
	// directories are automatically searched.
	...
	// has the same filename as the file in the local filesystem.
	// 
	void put_ExternalRefDirs(const uint16_t *newVal);
	...
	// 
	bool CanonicalizeFragment(const uint16_t *xml, const uint16_t *fragmentId, const uint16_t *version, const uint16_t *prefixList, bool withComments, CkString &outStr);
	// Applies XML canonicalization to a fragment of the passed-in XML, and returns the
	// canonicalized XML string. The fragmentId identifies the XML element where output
	// begins. It is the XML element having an "id" attribute equal to fragmentId. The version
	// may be one of the following:
	...
	// false, then XML comments are excluded from the output.
	// 
	const uint16_t *canonicalizeFragment(const uint16_t *xml, const uint16_t *fragmentId, const uint16_t *version, const uint16_t *prefixList, bool withComments);
	...
	// false, then XML comments are excluded from the output.
	// 
	bool CanonicalizeXml(const uint16_t *xml, const uint16_t *version, bool withComments, CkString &outStr);
	// Applies XML canonicalization to the passed-in XML, and returns the canonicalized
	// XML string. The version may be one of the following:
	//     "C14N" -- for Inclusive Canonical XML
	//     "EXCL_C14N" -- for Exclusive Canonical XML
	...
	// false, then XML comments are excluded from the output.
	// 
	const uint16_t *canonicalizeXml(const uint16_t *xml, const uint16_t *version, bool withComments);
	...
	bool GetCerts(CkStringArrayU &sa);
	...
	// signatures found.
	// 
	bool LoadSignature(const uint16_t *xmlSig);
	...
	// signatures found.
	// 
	bool LoadSignatureBd(CkBinDataU &binData);
	...
	// signatures found.
	// 
	bool LoadSignatureSb(CkStringBuilderU &sbXmlSig);
	...
	// Returns the URI of the Nth reference specified by index. (The 1st reference is at
	// index 0.) URI's beginning with a pound sign ('#') are internal references.
	const uint16_t *referenceUri(int index);
	...
	bool SetPublicKey(CkPublicKeyU &pubKey);

	// Provides the binary data for the external reference at index.
	bool SetRefDataBd(int index, CkBinDataU &binData);
	...
	// validation.
	bool SetRefDataFile(int index, const uint16_t *path);
	...
	bool SetRefDataSb(int index, CkStringBuilderU &sb, const uint16_t *charset);
	...
	bool UseCertVault(CkXmlCertVaultU &certVault);
