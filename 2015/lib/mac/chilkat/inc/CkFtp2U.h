// CkFtp2U.h: interface for the CkFtp2U class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkFtp2U
class CK_VISIBLE_PUBLIC CkFtp2U  : public CkClassWithCallbacksU
{
	public:
	bool m_cbOwned;
	...
	CkFtp2U(const CkFtp2U &);
	CkFtp2U &operator=(const CkFtp2U &);

    public:
	CkFtp2U(void);
	virtual ~CkFtp2U(void);

	

	static CkFtp2U *createNew(void);
	

	CkFtp2U(bool bCallbackOwned);
	static CkFtp2U *createNew(bool bCallbackOwned);
	...
	CkFtp2ProgressU *get_EventCallbackObject(void) const;
	void put_EventCallbackObject(CkFtp2ProgressU *progress);
	...
	// is updated in real-time and an application may periodically fetch and display
	// it's value while the download is in progress.
	const uint16_t *asyncBytesReceivedStr(void);
	...
	// is updated in real-time and an application may periodically fetch and display
	// it's value while the upload is in progress.
	const uint16_t *asyncBytesSentStr(void);
	...
	// indicated as a feature, then this property is automatically changed to "utf-8".
	// 
	const uint16_t *commandCharset(void);
	// Indicates the charset to be used for commands sent to the FTP server. The
	// command charset must match what the FTP server is expecting in order to
	// communicate non-English characters correctly. The default value of this property
	// is "ansi".
	...
	// indicated as a feature, then this property is automatically changed to "utf-8".
	// 
	void put_CommandCharset(const uint16_t *newVal);
	...
	//     without TLS).
	//     "private" means that the data connections will always be encrypted (TLS).
	const uint16_t *dataProtection(void);
	// Controls the data protection level for the data connections. Possible values are
	// "control", "clear", or "private".
	//     "control" is the default, and the data connections will be the same as for
	//     the control connection. If the control connection is SSL/TLS, then the data
	...
	//     without TLS).
	//     "private" means that the data connections will always be encrypted (TLS).
	void put_DataProtection(const uint16_t *newVal);
	...
	// indicated as a feature, then this property is automatically changed to "utf-8".
	// 
	const uint16_t *dirListingCharset(void);
	// Indicates the charset of the directory listings received from the FTP server.
	// The FTP2 client must interpret the directory listing bytes using the correct
	// character encoding in order to correctly receive non-English characters. The
	// default value of this property is "ansi".
	...
	// indicated as a feature, then this property is automatically changed to "utf-8".
	// 
	void put_DirListingCharset(const uint16_t *newVal);
	...
	// is only used as the argument to the PORT command that is sent to the server.
	// 
	const uint16_t *forcePortIpAddress(void);
	// If set, forces the IP address used in the PORT command for Active mode (i.e.
	// non-passive) data transfers. This string property should be set to the IP
	// address in dotted notation, such as "233.190.65.31".
	// 
	...
	// The domain name of the FTP server. May also use the IPv4 or IPv6 address in
	// string format.
	const uint16_t *hostname(void);
	// The domain name of the FTP server. May also use the IPv4 or IPv6 address in
	// string format.
	void put_Hostname(const uint16_t *newVal);
	...
	// ChangeRemoteDir, and then set the ListPattern equal to "*.txt".
	// 
	const uint16_t *listPattern(void);
	// A wildcard pattern, defaulting to "*" that determines the files and directories
	// included in the following properties and methods: GetDirCount, GetCreateTime,
	// GetFilename, GetIsDirectory, GetLastAccessTime, GetModifiedTime, GetSize.
	// 
	...
	// ChangeRemoteDir, and then set the ListPattern equal to "*.txt".
	// 
	void put_ListPattern(const uint16_t *newVal);
	...
	// The hostname of your FTP proxy, if a proxy server is used.
	const uint16_t *proxyHostname(void);
	// The hostname of your FTP proxy, if a proxy server is used.
	void put_ProxyHostname(const uint16_t *newVal);
	...
	// The username for authenticating with the FTP proxy server.
	const uint16_t *proxyUsername(void);
	// The username for authenticating with the FTP proxy server.
	void put_ProxyUsername(const uint16_t *newVal);
	...
	// SyncRemoteTree2 is called, this property contains the filepaths of the local
	// files that would be uploaded to the FTP server.
	const uint16_t *syncPreview(void);
	...
	// Username for logging into the FTP server. Defaults to "anonymous".
	const uint16_t *username(void);
	// Username for logging into the FTP server. Defaults to "anonymous".
	void put_Username(const uint16_t *newVal);
	...
	// 
	bool AppendFile(const uint16_t *localFilePath, const uint16_t *remoteFilePath);

	// Creates an asynchronous task to call the AppendFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AppendFileAsync(const uint16_t *localFilePath, const uint16_t *remoteFilePath);

	// Same as PutFileFromBinaryData, except the file on the FTP server is appended.
	bool AppendFileFromBinaryData(const uint16_t *remoteFilename, CkByteData &content);
	...
	// v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AppendFileFromBinaryDataAsync(const uint16_t *remoteFilename, CkByteData &content);

	// Same as PutFileFromTextData, except the file on the FTP server is appended.
	bool AppendFileFromTextData(const uint16_t *remoteFilename, const uint16_t *textData, const uint16_t *charset);

	// Creates an asynchronous task to call the AppendFileFromTextData method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AppendFileFromTextDataAsync(const uint16_t *remoteFilename, const uint16_t *textData, const uint16_t *charset);
	...
	// for the DirListingCharset property.
	// 
	bool ChangeRemoteDir(const uint16_t *remoteDirPath);

	// Creates an asynchronous task to call the ChangeRemoteDir method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ChangeRemoteDirAsync(const uint16_t *remoteDirPath);
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *CheckConnectionAsync(void);
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ClearControlChannelAsync(void);
	...
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ConvertToTlsAsync(void);
	...
	bool CreatePlan(const uint16_t *localDir, CkString &outStr);
	// Creates an "FTP plan" that lists the FTP operations that would be performed when
	// PutTree is called. Additionally, the PutPlan method executes an "FTP plan" and
	// logs each successful operation to a plan log file. If a large-scale upload is
	// interrupted, the PutPlan can be resumed, skipping over the operations already
	// listed in the plan log file.
	const uint16_t *createPlan(const uint16_t *localDir);

	// Creates an asynchronous task to call the CreatePlan method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *CreatePlanAsync(const uint16_t *localDir);
	...
	// for the DirListingCharset property.
	// 
	bool CreateRemoteDir(const uint16_t *remoteDirPath);

	// Creates an asynchronous task to call the CreateRemoteDir method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *CreateRemoteDirAsync(const uint16_t *remoteDirPath);
	...
	int DeleteMatching(const uint16_t *remotePattern);

	// Creates an asynchronous task to call the DeleteMatching method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DeleteMatchingAsync(const uint16_t *remotePattern);
	...
	// for the DirListingCharset property.
	// 
	bool DeleteRemoteFile(const uint16_t *remoteFilePath);

	// Creates an asynchronous task to call the DeleteRemoteFile method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DeleteRemoteFileAsync(const uint16_t *remoteFilePath);
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DetermineProxyMethodAsync(void);
	...
	// properties apply to this method.
	// 
	bool DownloadTree(const uint16_t *localRoot);

	// Creates an asynchronous task to call the DownloadTree method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DownloadTreeAsync(const uint16_t *localRoot);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkDateTimeU *GetCreateDtByName(const uint16_t *filename);

	// Creates an asynchronous task to call the GetCreateDtByName method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetCreateDtByNameAsync(const uint16_t *filename);
	...
	// 
	bool GetCreateTimeByName(const uint16_t *filename, SYSTEMTIME &outSysTime);
	...
	// last-modified date/time.
	// 
	bool GetCreateTimeByNameStr(const uint16_t *filename, CkString &outStr);
	// Returns the file-creation date/time (in RFC822 string format, such as "Tue, 25
	// Sep 2012 12:25:32 -0500") for a remote file by filename.
	// 
	// Note: The filename passed to this method must NOT include a path. Prior to calling
	...
	// last-modified date/time.
	// 
	const uint16_t *getCreateTimeByNameStr(const uint16_t *filename);
	// Returns the file-creation date/time (in RFC822 string format, such as "Tue, 25
	// Sep 2012 12:25:32 -0500") for a remote file by filename.
	// 
	// Note: The filename passed to this method must NOT include a path. Prior to calling
	...
	// last-modified date/time.
	// 
	const uint16_t *createTimeByNameStr(const uint16_t *filename);

	// Creates an asynchronous task to call the GetCreateTimeByNameStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetCreateTimeByNameStrAsync(const uint16_t *filename);
	...
	// directory. The first file/dir is at index 0, and the last one is at index
	// (GetDirCount()-1)
	const uint16_t *getCreateTimeStr(int index);
	// Returns the create time (in RFC822 string format, such as "Tue, 25 Sep 2012
	// 12:25:32 -0500") for the Nth file or sub-directory in the current remote
	// directory. The first file/dir is at index 0, and the last one is at index
	// (GetDirCount()-1)
	const uint16_t *createTimeStr(int index);

	// Creates an asynchronous task to call the GetCreateTimeStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetCreateTimeStrAsync(int index);

	// Returns the current remote directory.
	bool GetCurrentRemoteDir(CkString &outStr);
	// Returns the current remote directory.
	const uint16_t *getCurrentRemoteDir(void);
	// Returns the current remote directory.
	const uint16_t *currentRemoteDir(void);

	// Creates an asynchronous task to call the GetCurrentRemoteDir method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetCurrentRemoteDirAsync(void);
	...
	// for the DirListingCharset property.
	// 
	bool GetFile(const uint16_t *remoteFilePath, const uint16_t *localFilePath);

	// Creates an asynchronous task to call the GetFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileAsync(const uint16_t *remoteFilePath, const uint16_t *localFilePath);
	...
	// for the DirListingCharset property.
	// 
	bool GetFileBd(const uint16_t *remoteFilePath, CkBinDataU &binData);

	// Creates an asynchronous task to call the GetFileBd method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileBdAsync(const uint16_t *remoteFilePath, CkBinDataU &binData);
	...
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFilenameAsync(int index);
	...
	// for the DirListingCharset property.
	// 
	bool GetFileSb(const uint16_t *remoteFilePath, const uint16_t *charset, CkStringBuilderU &sb);

	// Creates an asynchronous task to call the GetFileSb method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileSbAsync(const uint16_t *remoteFilePath, const uint16_t *charset, CkStringBuilderU &sb);
	...
	bool GetFileToStream(const uint16_t *remoteFilePath, CkStreamU &toStream);

	// Creates an asynchronous task to call the GetFileToStream method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileToStreamAsync(const uint16_t *remoteFilePath, CkStreamU &toStream);
	...
	// quality and accuracy of the various date/time values that are returned.
	// 
	const uint16_t *getGroup(int index);
	// Returns group name, if available, for the Nth file. If empty, then no group
	// information is available.
	// 
	// Note: When MLSD is used to get directory listings, it is likely that the owner
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetIsDirectoryAsync(int index);
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetIsSymbolicLinkAsync(int index);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkDateTimeU *GetLastModDtByName(const uint16_t *filename);

	// Creates an asynchronous task to call the GetLastModDtByName method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetLastModDtByNameAsync(const uint16_t *filename);
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	bool GetLastModifiedTimeByName(const uint16_t *filename, SYSTEMTIME &outSysTime);
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	bool GetLastModifiedTimeByNameStr(const uint16_t *filename, CkString &outStr);
	// Returns a remote file's last-modified date/time in RFC822 string format, such as
	// "Tue, 25 Sep 2012 12:25:32 -0500".
	// 
	// Note: The filename passed to this method must NOT include a path. Prior to calling
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	const uint16_t *getLastModifiedTimeByNameStr(const uint16_t *filename);
	// Returns a remote file's last-modified date/time in RFC822 string format, such as
	// "Tue, 25 Sep 2012 12:25:32 -0500".
	// 
	// Note: The filename passed to this method must NOT include a path. Prior to calling
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	const uint16_t *lastModifiedTimeByNameStr(const uint16_t *filename);
	...
	// v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetLastModifiedTimeByNameStrAsync(const uint16_t *filename);
	...
	// remote directory. The first file/dir is at index 0, and the last one is at index
	// (GetDirCount()-1)
	const uint16_t *getLastModifiedTimeStr(int index);
	// Returns the last modified date/time (in RFC822 string format, such as "Tue, 25
	// Sep 2012 12:25:32 -0500") for the Nth file or sub-directory in the current
	// remote directory. The first file/dir is at index 0, and the last one is at index
	// (GetDirCount()-1)
	const uint16_t *lastModifiedTimeStr(int index);

	// Creates an asynchronous task to call the GetLastModifiedTimeStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetLastModifiedTimeStrAsync(int index);
	...
	// quality and accuracy of the various date/time values that are returned.
	// 
	const uint16_t *getOwner(int index);
	// Returns owner name, if available, for the Nth file. If empty, then no owner
	// information is available.
	// 
	// Note: When MLSD is used to get directory listings, it is likely that the owner
	...
	// 10) - -- Always a dash.
	// 
	const uint16_t *getPermissions(int index);
	// Returns permissions information, if available, for the Nth file. If empty, then
	// no permissions information is available. The value returned by the GetPermType
	// method defines the content and format of the permissions string returned by this
	// method. Possible permission types are "mlsd", "unix", "netware", "openvms", and
	...
	// 10) - -- Always a dash.
	// 
	const uint16_t *permissions(int index);

	// Creates an asynchronous task to call the GetPermissions method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetPermissionsAsync(int index);
	...
	// the GetPermissions method. Possible values are "mlsd", "unix", "netware",
	// "openvms", and "batchStatusFlags".
	const uint16_t *getPermType(int index);
	// Returns the type of permissions information that is available for the Nth file.
	// If empty, then no permissions information is available. The value returned by
	// this method defines the content and format of the permissions string returned by
	// the GetPermissions method. Possible values are "mlsd", "unix", "netware",
	// "openvms", and "batchStatusFlags".
	const uint16_t *permType(int index);

	// Creates an asynchronous task to call the GetPermType method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetPermTypeAsync(int index);

	// Downloads the contents of a remote file into a byte array.
	bool GetRemoteFileBinaryData(const uint16_t *remoteFilename, CkByteData &outData);

	// Creates an asynchronous task to call the GetRemoteFileBinaryData method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetRemoteFileBinaryDataAsync(const uint16_t *remoteFilename);
	...
	bool GetRemoteFileTextC(const uint16_t *remoteFilename, const uint16_t *charset, CkString &outStr);
	// Downloads a text file directly into a string variable. The character encoding of
	// the text file is specified by the charset argument, which is a value such as utf-8,
	// iso-8859-1, Shift_JIS, etc.
	const uint16_t *getRemoteFileTextC(const uint16_t *remoteFilename, const uint16_t *charset);
	// Downloads a text file directly into a string variable. The character encoding of
	// the text file is specified by the charset argument, which is a value such as utf-8,
	// iso-8859-1, Shift_JIS, etc.
	const uint16_t *remoteFileTextC(const uint16_t *remoteFilename, const uint16_t *charset);

	// Creates an asynchronous task to call the GetRemoteFileTextC method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetRemoteFileTextCAsync(const uint16_t *remoteFilename, const uint16_t *charset);
	...
	// 
	bool GetRemoteFileTextData(const uint16_t *remoteFilename, CkString &outStr);
	// Downloads the content of a remote text file directly into an in-memory string.
	// 
	// Note: If the remote text file does not use the ANSI character encoding, call
	// GetRemoteFileTextC instead, which allows for the character encoding to be
	// specified so that characters are properly interpreted.
	// 
	const uint16_t *getRemoteFileTextData(const uint16_t *remoteFilename);
	// Downloads the content of a remote text file directly into an in-memory string.
	// 
	// Note: If the remote text file does not use the ANSI character encoding, call
	// GetRemoteFileTextC instead, which allows for the character encoding to be
	// specified so that characters are properly interpreted.
	// 
	const uint16_t *remoteFileTextData(const uint16_t *remoteFilename);

	// Creates an asynchronous task to call the GetRemoteFileTextData method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetRemoteFileTextDataAsync(const uint16_t *remoteFilename);
	...
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetSizeAsync(int index);
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	int GetSizeByName(const uint16_t *filename);

	// Creates an asynchronous task to call the GetSizeByName method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetSizeByNameAsync(const uint16_t *filename);
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	__int64 GetSizeByName64(const uint16_t *filename);
	...
	// directory. This is helpful for cases when the file size (in bytes) is greater
	// than what can fit in a 32-bit integer.
	const uint16_t *getSizeStr(int index);
	// Returns the size in decimal string format of the Nth remote file in the current
	// directory. This is helpful for cases when the file size (in bytes) is greater
	// than what can fit in a 32-bit integer.
	const uint16_t *sizeStr(int index);

	// Creates an asynchronous task to call the GetSizeStr method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetSizeStrAsync(int index);
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	bool GetSizeStrByName(const uint16_t *filename, CkString &outStr);
	// Returns the size of a remote file as a string. This is helpful when file a file
	// size is greater than what can fit in a 32-bit integer.
	// 
	// Note: The filename passed to this method must NOT include a path. Prior to calling
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	const uint16_t *getSizeStrByName(const uint16_t *filename);
	// Returns the size of a remote file as a string. This is helpful when file a file
	// size is greater than what can fit in a 32-bit integer.
	// 
	// Note: The filename passed to this method must NOT include a path. Prior to calling
	...
	// value of ListPattern is "*", which will match all filenames.)
	// 
	const uint16_t *sizeStrByName(const uint16_t *filename);

	// Creates an asynchronous task to call the GetSizeStrByName method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetSizeStrByNameAsync(const uint16_t *filename);
	...
	bool GetTextDirListing(const uint16_t *pattern, CkString &outStrRawListing);
	// Returns a listing of the files and directories in the current directory matching
	// the pattern. Passing "*.*" will return all the files and directories.
	const uint16_t *getTextDirListing(const uint16_t *pattern);
	// Returns a listing of the files and directories in the current directory matching
	// the pattern. Passing "*.*" will return all the files and directories.
	const uint16_t *textDirListing(const uint16_t *pattern);

	// Creates an asynchronous task to call the GetTextDirListing method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetTextDirListingAsync(const uint16_t *pattern);
	...
	// 
	bool GetXmlDirListing(const uint16_t *pattern, CkString &outStrXmlListing);
	// Returns (in XML format) the files and directories in the current directory
	// matching the pattern. Passing "*.*" will return all the files and directories.
	// 
	// Note: The lastModTime XML elements contain date/time information in the local
	...
	// server cannot be known, especially for older FTP server implementations.
	// 
	const uint16_t *getXmlDirListing(const uint16_t *pattern);
	// Returns (in XML format) the files and directories in the current directory
	// matching the pattern. Passing "*.*" will return all the files and directories.
	// 
	// Note: The lastModTime XML elements contain date/time information in the local
	...
	// server cannot be known, especially for older FTP server implementations.
	// 
	const uint16_t *xmlDirListing(const uint16_t *pattern);

	// Creates an asynchronous task to call the GetXmlDirListing method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetXmlDirListingAsync(const uint16_t *pattern);
	...
	// solved).
	// 
	bool LargeFileUpload(const uint16_t *localPath, const uint16_t *remotePath, int chunkSize);

	// Creates an asynchronous task to call the LargeFileUpload method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *LargeFileUploadAsync(const uint16_t *localPath, const uint16_t *remotePath, int chunkSize);
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *LoginAfterConnectOnlyAsync(void);
	...
	// 
	int MGetFiles(const uint16_t *remotePattern, const uint16_t *localDir);

	// Creates an asynchronous task to call the MGetFiles method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *MGetFilesAsync(const uint16_t *remotePattern, const uint16_t *localDir);
	...
	// from the XML log.
	int MPutFiles(const uint16_t *pattern);

	// Creates an asynchronous task to call the MPutFiles method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *MPutFilesAsync(const uint16_t *pattern);
	...
	// 
	bool NlstXml(const uint16_t *remoteDirPattern, CkString &outStr);
	// Sends an NLST command to the FTP server and returns the results in XML format.
	// The NLST command returns a list of filenames in the given directory (matching
	// the pattern). The remoteDirPattern should be a pattern such as "*", "*.*", "*.txt",
	// "subDir/*.xml", etc.
	...
	// filename_or_dir_1filename_or_dir_2filename_or_dir_3filename_or_dir_4...
	// 
	const uint16_t *nlstXml(const uint16_t *remoteDirPattern);

	// Creates an asynchronous task to call the NlstXml method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *NlstXmlAsync(const uint16_t *remoteDirPattern);
	...
	// for the DirListingCharset property.
	// 
	bool PutFile(const uint16_t *localFilePath, const uint16_t *remoteFilePath);

	// Creates an asynchronous task to call the PutFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *PutFileAsync(const uint16_t *localFilePath, const uint16_t *remoteFilePath);
	...
	// for the DirListingCharset property.
	// 
	bool PutFileBd(CkBinDataU &binData, const uint16_t *remoteFilePath);

	// Creates an asynchronous task to call the PutFileBd method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *PutFileBdAsync(CkBinDataU &binData, const uint16_t *remoteFilePath);

	// Creates a file on the remote server containing the data passed in a byte array.
	bool PutFileFromBinaryData(const uint16_t *remoteFilename, CkByteData &content);

	// Creates an asynchronous task to call the PutFileFromBinaryData method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *PutFileFromBinaryDataAsync(const uint16_t *remoteFilename, CkByteData &content);

	// Creates a file on the remote server containing the data passed in a string.
	bool PutFileFromTextData(const uint16_t *remoteFilename, const uint16_t *textData, const uint16_t *charset);

	// Creates an asynchronous task to call the PutFileFromTextData method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *PutFileFromTextDataAsync(const uint16_t *remoteFilename, const uint16_t *textData, const uint16_t *charset);
	...
	// for the DirListingCharset property.
	// 
	bool PutFileSb(CkStringBuilderU &sb, const uint16_t *charset, bool includeBom, const uint16_t *remoteFilePath);

	// Creates an asynchronous task to call the PutFileSb method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *PutFileSbAsync(CkStringBuilderU &sb, const uint16_t *charset, bool includeBom, const uint16_t *remoteFilePath);
	...
	bool PutPlan(const uint16_t *plan, const uint16_t *alreadyDoneFilename);

	// Creates an asynchronous task to call the PutPlan method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *PutPlanAsync(const uint16_t *plan, const uint16_t *alreadyDoneFilename);
	...
	// for the DirListingCharset property.
	// 
	bool RemoveRemoteDir(const uint16_t *remoteDirPath);

	// Creates an asynchronous task to call the RemoveRemoteDir method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *RemoveRemoteDirAsync(const uint16_t *remoteDirPath);
	...
	// 
	bool RenameRemoteFile(const uint16_t *existingRemoteFilePath, const uint16_t *newRemoteFilePath);

	// Creates an asynchronous task to call the RenameRemoteFile method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *RenameRemoteFileAsync(const uint16_t *existingRemoteFilePath, const uint16_t *newRemoteFilePath);

	// Sends an raw command to the FTP server and returns the raw response.
	bool SendCommand(const uint16_t *cmd, CkString &outReply);
	// Sends an raw command to the FTP server and returns the raw response.
	const uint16_t *sendCommand(const uint16_t *cmd);

	// Creates an asynchronous task to call the SendCommand method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SendCommandAsync(const uint16_t *cmd);
	...
	// 2012 12:25:32 -0500".
	// 
	void SetOldestDateStr(const uint16_t *oldestDateTimeStr);
	...
	// 
	bool SetOption(const uint16_t *option);
	...
	// Chilkat blog below:
	bool SetRemoteFileDateTime(SYSTEMTIME &dt, const uint16_t *remoteFilename);
	...
	// 
	bool SetRemoteFileDateTimeStr(const uint16_t *dateTimeStr, const uint16_t *remoteFilename);

	// Creates an asynchronous task to call the SetRemoteFileDateTimeStr method with
	// the arguments provided. (Async methods are available starting in Chilkat
	// v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetRemoteFileDateTimeStrAsync(const uint16_t *dateTimeStr, const uint16_t *remoteFilename);

	// Sets the last-modified date/time of a file on the FTP server. Important: Not all
	// FTP servers support this functionality. Please see the information at the
	// Chilkat blog below:
	bool SetRemoteFileDt(CkDateTimeU &dt, const uint16_t *remoteFilename);

	// Creates an asynchronous task to call the SetRemoteFileDt method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetRemoteFileDtAsync(CkDateTimeU &dt, const uint16_t *remoteFilename);
	...
	// 
	void SetSslCertRequirement(const uint16_t *reqName, const uint16_t *reqValue);
	...
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetTypeAsciiAsync(void);
	...
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetTypeBinaryAsync(void);
	...
	bool Site(const uint16_t *siteCommand);

	// Creates an asynchronous task to call the Site method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SiteAsync(const uint16_t *siteCommand);
	...
	// 
	bool SyncDeleteRemote(const uint16_t *localRoot);

	// Creates an asynchronous task to call the SyncDeleteRemote method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SyncDeleteRemoteAsync(const uint16_t *localRoot);
	...
	// SyncedFiles property.
	// 
	bool SyncLocalDir(const uint16_t *localRoot, int mode);

	// Creates an asynchronous task to call the SyncLocalDir method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SyncLocalDirAsync(const uint16_t *localRoot, int mode);
	...
	// 
	bool SyncLocalTree(const uint16_t *localRoot, int mode);

	// Creates an asynchronous task to call the SyncLocalTree method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SyncLocalTreeAsync(const uint16_t *localRoot, int mode);
	...
	// SyncedFiles property.
	// 
	bool SyncRemoteTree(const uint16_t *localRoot, int mode);

	// Creates an asynchronous task to call the SyncRemoteTree method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SyncRemoteTreeAsync(const uint16_t *localRoot, int mode);
	...
	// SyncedFiles property.
	// 
	bool SyncRemoteTree2(const uint16_t *localDirPath, int mode, bool bDescend, bool bPreviewOnly);

	// Creates an asynchronous task to call the SyncRemoteTree2 method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SyncRemoteTree2Async(const uint16_t *localDirPath, int mode, bool bDescend, bool bPreviewOnly);
