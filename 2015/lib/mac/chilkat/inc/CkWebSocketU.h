// CkWebSocketU.h: interface for the CkWebSocketU class.
//
//////////////////////////////////////////////////////////////////////

// This header is generated for Chilkat 9.5.0.89

#ifndef _CkWebSocketU_H
#define _CkWebSocketU_H
...
// CLASS: CkWebSocketU
class CK_VISIBLE_PUBLIC CkWebSocketU  : public CkClassWithCallbacksU
{
	public:
	bool m_cbOwned;
	...
	CkWebSocketU(const CkWebSocketU &);
	CkWebSocketU &operator=(const CkWebSocketU &);

    public:
	CkWebSocketU(void);
	virtual ~CkWebSocketU(void);

	

	static CkWebSocketU *createNew(void);
	

	CkWebSocketU(bool bCallbackOwned);
	static CkWebSocketU *createNew(bool bCallbackOwned);
	...
	// The reason string received with the Close frame, if any.
	const uint16_t *closeReason(void);
	...
	// Initially this property is set to the empty string because nothing has yet been
	// received.
	const uint16_t *frameOpcode(void);
	...
	// Returns the accumulated received frame data as a string. Calling GetFrameData
	// clears the internal receive buffer.
	const uint16_t *getFrameData(void);
	// Returns the accumulated received frame data as a string. Calling GetFrameData
	// clears the internal receive buffer.
	const uint16_t *frameData(void);
	...
	// 
	bool GetFrameDataBd(CkBinDataU &binData);
	...
	// Calling this method clears the internal receive buffer.
	// 
	bool GetFrameDataSb(CkStringBuilderU &sb);
	...
	// 
	bool SendClose(bool includeStatus, int statusCode, const uint16_t *reason);

	// Creates an asynchronous task to call the SendClose method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SendCloseAsync(bool includeStatus, int statusCode, const uint16_t *reason);
	...
	bool SendFrame(const uint16_t *stringToSend, bool finalFrame);

	// Creates an asynchronous task to call the SendFrame method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SendFrameAsync(const uint16_t *stringToSend, bool finalFrame);
	...
	bool SendFrameBd(CkBinDataU &bdToSend, bool finalFrame);

	// Creates an asynchronous task to call the SendFrameBd method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SendFrameBdAsync(CkBinDataU &bdToSend, bool finalFrame);
	...
	bool SendFrameSb(CkStringBuilderU &sbToSend, bool finalFrame);

	// Creates an asynchronous task to call the SendFrameSb method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SendFrameSbAsync(CkStringBuilderU &sbToSend, bool finalFrame);
	...
	bool SendPing(const uint16_t *pingData);

	// Creates an asynchronous task to call the SendPing method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SendPingAsync(const uint16_t *pingData);
	...
	// options, etc.
	bool UseConnection(CkRestU &connection);
