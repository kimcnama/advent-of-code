// CkEmailU.h: interface for the CkEmailU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkEmailU
class CK_VISIBLE_PUBLIC CkEmailU  : public CkUtf16Base
{
	...
	CkEmailU(const CkEmailU &);
	CkEmailU &operator=(const CkEmailU &);

    public:
	CkEmailU(void);
	virtual ~CkEmailU(void);

	

	static CkEmailU *createNew(void);
	...
	// be used to access a specific body. The HasHtmlBody and HasPlainTextBody methods
	// can be used to determine the presence of a body.
	const uint16_t *body(void);
	// The body of the email. If the email has both HTML and plain-text bodies, this
	// property returns the HTML body. The GetHtmlBody and GetPlainTextBody methods can
	// be used to access a specific body. The HasHtmlBody and HasPlainTextBody methods
	// can be used to determine the presence of a body.
	void put_Body(const uint16_t *newVal);
	...
	// the BounceAddress property served this function.
	// 
	void put_BounceAddress(const uint16_t *newVal);
	...
	// The date/time from the "Date" header in the UTC/GMT timezone in RFC822 string
	// form.
	const uint16_t *emailDateStr(void);
	// The date/time from the "Date" header in the UTC/GMT timezone in RFC822 string
	// form.
	void put_EmailDateStr(const uint16_t *newVal);
	...
	// If the email was received encrypted, this contains the details of the
	// certificate used for encryption.
	const uint16_t *encryptedBy(void);
	...
	// file and send the email to each recipient. Emails can be sent individually, or
	// with BCC, 100 recipients at a time. (see the MailMan.SendIndividual property).
	const uint16_t *fileDistList(void);
	// Set this property to send an email to a list of recipients stored in a plain
	// text file. The file format is simple: one recipient per line, no comments
	// allowed, blank lines are ignored.Setting this property is equivalent to adding a
	// "CKX-FileDistList"header field to the email. Chilkat Mail treats header fields
	...
	// file and send the email to each recipient. Emails can be sent individually, or
	// with BCC, 100 recipients at a time. (see the MailMan.SendIndividual property).
	void put_FileDistList(const uint16_t *newVal);
	...
	// sender is to be different, then set the Sender property to a different email
	// address.
	const uint16_t *ck_from(void);
	// The combined name and email address of the sender, such as "John Smith" . This
	// is the content that will be placed in the From: header field. If the actual
	// sender is to be different, then set the Sender property to a different email
	// address.
	void put_From(const uint16_t *newVal);

	// The email address of the sender.
	void get_FromAddress(CkString &str);
	// The email address of the sender.
	const uint16_t *fromAddress(void);
	// The email address of the sender.
	void put_FromAddress(const uint16_t *newVal);

	// The name of the sender.
	void get_FromName(CkString &str);
	// The name of the sender.
	const uint16_t *fromName(void);
	// The name of the sender.
	void put_FromName(const uint16_t *newVal);
	...
	// The date/time found in the "Date" header field returned in the local timezone in
	// RFC822 string form.
	const uint16_t *localDateStr(void);
	// The date/time found in the "Date" header field returned in the local timezone in
	// RFC822 string form.
	void put_LocalDateStr(const uint16_t *newVal);

	// Identifies the email software that sent the email.
	void get_Mailer(CkString &str);
	// Identifies the email software that sent the email.
	const uint16_t *mailer(void);
	// Identifies the email software that sent the email.
	void put_Mailer(const uint16_t *newVal);
	...
	// Selects the hash algorithm for use within OAEP padding when encrypting email
	// using RSAES-OAEP. The valid choices are "sha1", "sha256", "sha384", "sha512",
	const uint16_t *oaepHash(void);
	// Selects the hash algorithm for use within OAEP padding when encrypting email
	// using RSAES-OAEP. The valid choices are "sha1", "sha256", "sha384", "sha512",
	void put_OaepHash(const uint16_t *newVal);
	...
	// Selects the MGF hash algorithm for use within OAEP padding when encrypting email
	// using RSAES-OAEP. The valid choices are "sha1", "sha256", "sha384", "sha512",
	const uint16_t *oaepMgfHash(void);
	// Selects the MGF hash algorithm for use within OAEP padding when encrypting email
	// using RSAES-OAEP. The valid choices are "sha1", "sha256", "sha384", "sha512",
	void put_OaepMgfHash(const uint16_t *newVal);
	...
	// selects the underlying symmetric encryption algorithm. Possible values are:
	// "aes", "des", "3des", and "rc2". The default value is "aes".
	const uint16_t *pkcs7CryptAlg(void);
	// When an email is sent encrypted (using PKCS7 public-key encryption), this
	// selects the underlying symmetric encryption algorithm. Possible values are:
	// "aes", "des", "3des", and "rc2". The default value is "aes".
	void put_Pkcs7CryptAlg(const uint16_t *newVal);
	...
	// contains Greek characters, then the preferred charset is ignored.
	// 
	const uint16_t *preferredCharset(void);
	// Only applies when building an email with non-English characters where the
	// charset is not explicitly set. The Chilkat email component will automatically
	// choose a charset based on the languages found within an email (if the charset is
	// not already specified within the MIME or explicitly specified by setting the
	...
	// contains Greek characters, then the preferred charset is ignored.
	// 
	void put_PreferredCharset(const uint16_t *newVal);
	...
	// Sets the "Reply-To" header field to the specified email address.
	const uint16_t *replyTo(void);
	// Sets the "Reply-To" header field to the specified email address.
	void put_ReplyTo(const uint16_t *newVal);
	...
	// always exist), call the GetHeaderField method instead.
	// 
	void put_Sender(const uint16_t *newVal);
	...
	// not apply for ECC or DSA private keys.
	// 
	const uint16_t *signingAlg(void);
	// Selects the signature algorithm to be used when sending signed (PKCS7) email.
	// The default value is "PKCS1-v1_5". This can be set to "RSASSA-PSS" (or simply
	// "pss") to use the RSASSA-PSS signature scheme.
	// 
	...
	// not apply for ECC or DSA private keys.
	// 
	void put_SigningAlg(const uint16_t *newVal);
	...
	// Selects the underlying hash algorithm used when sending signed (PKCS7) email.
	// Possible values are "sha1", "sha256", "sha384", "sha512", "md5", and "md2".
	const uint16_t *signingHashAlg(void);
	// Selects the underlying hash algorithm used when sending signed (PKCS7) email.
	// Possible values are "sha1", "sha256", "sha384", "sha512", "md5", and "md2".
	void put_SigningHashAlg(const uint16_t *newVal);
	...
	// The email subject.
	const uint16_t *subject(void);
	// The email subject.
	void put_Subject(const uint16_t *newVal);
	...
	bool AddAttachmentBd(const uint16_t *filename, CkBinDataU &binData, const uint16_t *contentType);
	...
	void AddAttachmentHeader(int index, const uint16_t *fieldName, const uint16_t *fieldValue);
	...
	// 
	bool AddBcc(const uint16_t *friendlyName, const uint16_t *emailAddress);
	...
	// 
	bool AddCC(const uint16_t *friendlyName, const uint16_t *emailAddress);

	// Adds an attachment directly from data in memory to the email.
	bool AddDataAttachment(const uint16_t *fileName, CkByteData &content);

	// Adds an attachment to an email from in-memory data. Same as AddDataAttachment
	// but allows the content-type to be specified.
	bool AddDataAttachment2(const uint16_t *fileName, CkByteData &content, const uint16_t *contentType);
	...
	// 
	bool AddEncryptCert(CkCertU &cert);
	...
	bool AddFileAttachment(const uint16_t *path, CkString &outStrContentType);
	// Adds a file as an attachment to the email. Returns the MIME content-type of the
	// attachment, which is inferred based on the filename extension.
	const uint16_t *addFileAttachment(const uint16_t *path);

	// Same as AddFileAttachment, but the content type can be explicitly specified.
	bool AddFileAttachment2(const uint16_t *path, const uint16_t *contentType);
	...
	// 
	void AddHeaderField(const uint16_t *fieldName, const uint16_t *fieldValue);
	...
	void AddHeaderField2(const uint16_t *fieldName, const uint16_t *fieldValue);
	...
	bool AddHtmlAlternativeBody(const uint16_t *body);
	...
	bool AddiCalendarAlternativeBody(const uint16_t *icalContent, const uint16_t *methodName);
	...
	// if successful.
	bool AddMultipleBcc(const uint16_t *commaSeparatedAddresses);
	...
	// successful.
	bool AddMultipleCC(const uint16_t *commaSeparatedAddresses);
	...
	bool AddMultipleTo(const uint16_t *commaSeparatedAddresses);
	...
	bool AddPlainTextAlternativeBody(const uint16_t *body);

	// Adds a related item using the contents of a BinData object. Returns the
	// Content-ID for the newly added relted item.
	bool AddRelatedBd(const uint16_t *filename, CkBinDataU &binData, CkString &outStr);
	// Adds a related item using the contents of a BinData object. Returns the
	// Content-ID for the newly added relted item.
	const uint16_t *addRelatedBd(const uint16_t *filename, CkBinDataU &binData);
	...
	// attribute.
	bool AddRelatedBd2(CkBinDataU &binData, const uint16_t *fileNameInHtml);
	...
	// the MHTML standard.)
	bool AddRelatedData(const uint16_t *fileName, CkByteData &inData, CkString &outStr);
	// Adds the memory data as a related item to the email and returns the Content-ID.
	// Emails formatted in HTML can include images with this call and internally
	// reference the image through a "cid"hyperlink. (Chilkat Email.NET fully supports
	// the MHTML standard.)
	const uint16_t *addRelatedData(const uint16_t *fileName, CkByteData &inData);
	...
	void AddRelatedData2(CkByteData &inData, const uint16_t *fileNameInHtml);

#if !defined(CHILKAT_MONO)
	// The same as AddRelatedData2, except the data is passed in as a "const unsigned
	// char *" with the byte count in szBytes.
	void AddRelatedData2P(const void *pBytes, unsigned long szBytes, const uint16_t *fileNameInHtml);
#endif
	...
	bool AddRelatedDataP(const uint16_t *nameInHtml, const void *pBytes, unsigned long szBytes, CkString &outStrContentId);
	// The same as AddRelatedData, except the data is passed in as a "const unsigned
	// char *" with the byte count in szBytes. The Content-ID assigned to the related item
	// is returned (in ARG4 for the upper-case alternative for this method).
	const uint16_t *addRelatedDataP(const uint16_t *nameInHtml, const void *pBytes, unsigned long szBytes);
#endif
	...
	// standard.)
	bool AddRelatedFile(const uint16_t *path, CkString &outStrContentID);
	// Adds the contents of a file to the email and returns the Content-ID. Emails
	// formatted in HTML can include images with this call and internally reference the
	// image through a "cid" hyperlink. (Chilkat Email.NET fully supports the MHTML
	// standard.)
	const uint16_t *addRelatedFile(const uint16_t *path);
	...
	// 
	bool AddRelatedFile2(const uint16_t *filenameOnDisk, const uint16_t *filenameInHtml);

	// Adds or replaces a MIME header field in one of the email's related items. If the
	// header field does not exist, it is added. Otherwise it is replaced.
	void AddRelatedHeader(int index, const uint16_t *fieldName, const uint16_t *fieldValue);
	...
	bool AddRelatedString(const uint16_t *nameInHtml, const uint16_t *str, const uint16_t *charset, CkString &outCid);
	// Adds a related item to the email. A related item is typically an image or style
	// sheet referenced by an HTML tag within the HTML email body. The contents of the
	// related item are passed str. nameInHtml specifies the filename that should be used
	// within the HTML, and not an actual filename on the local filesystem. charset
	// specifies the charset that should be used for the text content of the related
	// item. Returns the content-ID generated for the added item.
	const uint16_t *addRelatedString(const uint16_t *nameInHtml, const uint16_t *str, const uint16_t *charset);
	...
	// "Shift_JIS", etc.
	void AddRelatedString2(const uint16_t *fileNameInHtml, const uint16_t *content, const uint16_t *charset);

	// Adds an attachment directly from a string in memory to the email.
	bool AddStringAttachment(const uint16_t *path, const uint16_t *content);
	...
	// 
	bool AddStringAttachment2(const uint16_t *path, const uint16_t *content, const uint16_t *charset);
	...
	// 
	bool AddTo(const uint16_t *friendlyName, const uint16_t *emailAddress);
	...
	bool AesDecrypt(const uint16_t *password);
	...
	bool AesEncrypt(const uint16_t *password);

	// Appends a string to the plain-text body.
	void AppendToBody(const uint16_t *str);
	...
	//     seen as attachments.
	// 
	bool ApplyFixups(const uint16_t *fixups);

	// Please see the examples at the following pages for detailed information:
	bool AspUnpack(const uint16_t *prefix, const uint16_t *saveDir, const uint16_t *urlPath, bool cleanFiles);

	// Please see the examples at the following pages for detailed information:
	bool AspUnpack2(const uint16_t *prefix, const uint16_t *saveDir, const uint16_t *urlPath, bool cleanFiles, CkByteData &outHtml);

	// Attaches copy of the email to this email object. The attached email will be
	// encapsulated in an message/rfc822 sub-part.
	bool AttachEmail(CkEmailU &email);
	...
	// B-Encoded string.
	bool BEncodeBytes(CkByteData &inData, const uint16_t *charset, CkString &outEncodedStr);
	// Takes a byte array of multibyte (non-Unicode) data and returns a Unicode
	// B-Encoded string.
	const uint16_t *bEncodeBytes(CkByteData &inData, const uint16_t *charset);
	...
	// Unicode string.
	bool BEncodeString(const uint16_t *str, const uint16_t *charset, CkString &outEncodedStr);
	// Takes a Unicode string, converts it to the charset specified in the 2nd
	// parameter, B-Encodes the converted multibyte data, and returns the encoded
	// Unicode string.
	const uint16_t *bEncodeString(const uint16_t *str, const uint16_t *charset);
	...
	// 
	bool ComputeGlobalKey(const uint16_t *encoding, bool bFold, CkString &outStr);
	// Important: New programs should ComputeGlobalKey2 instead. This method did not
	// adequately canonicalize the string passed to the digest-MD5 hash and therefore
	// different versions of Chilkat may produce different results with this method.
	// 
	...
	// an XOR to produce a shorter key.
	// 
	const uint16_t *computeGlobalKey(const uint16_t *encoding, bool bFold);
	...
	bool ComputeGlobalKey2(const uint16_t *encoding, bool bFold, CkString &outStr);
	// Computes a global unique key for the email. The key is created by a digest-MD5
	// hash of the concatenation of the following:
	// messageID + CRLF + subject + CRLF + from + CRLF + date + CRLF + recipientAddrs
	// 
	...
	// encoding: base64, hex, url, etc. If bFold is true, then the 16-byte MD5 hash is
	// folded to 8 bytes with an XOR to produce a shorter key.
	const uint16_t *computeGlobalKey2(const uint16_t *encoding, bool bFold);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkEmailU *CreateDsn(const uint16_t *humanReadableMessage, const uint16_t *xmlStatusFields, bool bHeaderOnly);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkEmailU *CreateMdn(const uint16_t *humanReadableMessage, const uint16_t *xmlStatusFields, bool bHeaderOnly);
	...
	bool CreateTempMht(const uint16_t *inFilename, CkString &outPath);
	// Saves the email to a temporary MHT file so that a WebBrowser control can
	// navigate to it and display it. If fileName is empty, a temporary filename is
	// generated and returned. If fileName is non-empty, then it will be created or
	// overwritten, and the input filename is simply returned.The MHT file that is
	...
	// is converted to HTML using pre-formatted text ("pre" HTML tags) allowing it to
	// be displayed correctly in a WebBrowser.
	const uint16_t *createTempMht(const uint16_t *inFilename);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkCertU *FindIssuer(CkCertU &cert);
	...
	// Generates a unique filename for this email. The filename will be different each
	// time the method is called.
	const uint16_t *generateFilename(void);
	...
	// of alternative bodies present. Use the GetHtmlBody and GetPlainTextBody methods
	// to easily get the HTML or plain text alternative bodies.
	const uint16_t *getAlternativeBody(int index);
	// Returns the Nth alternative body. The NumAlternatives property tells the number
	// of alternative bodies present. Use the GetHtmlBody and GetPlainTextBody methods
	// to easily get the HTML or plain text alternative bodies.
	const uint16_t *alternativeBody(int index);
	...
	bool GetAlternativeBodyBd(int index, CkBinDataU &binData);

	// Returns the alternative body by content-type, such as "text/plain", "text/html",
	// "text/xml", etc.
	bool GetAlternativeBodyByContentType(const uint16_t *contentType, CkString &outStr);
	// Returns the alternative body by content-type, such as "text/plain", "text/html",
	// "text/xml", etc.
	const uint16_t *getAlternativeBodyByContentType(const uint16_t *contentType);
	// Returns the alternative body by content-type, such as "text/plain", "text/html",
	// "text/xml", etc.
	const uint16_t *alternativeBodyByContentType(const uint16_t *contentType);
	...
	// Returns the content type of the Nth alternative body. The NumAlternatives
	// property tells the number of alternative bodies present.
	const uint16_t *getAlternativeContentType(int index);
	// Returns the content type of the Nth alternative body. The NumAlternatives
	// property tells the number of alternative bodies present.
	const uint16_t *alternativeContentType(int index);

	// Returns the value of a header field within the Nth alternative body's MIME
	// sub-part.
	bool GetAltHeaderField(int index, const uint16_t *fieldName, CkString &outStr);
	// Returns the value of a header field within the Nth alternative body's MIME
	// sub-part.
	const uint16_t *getAltHeaderField(int index, const uint16_t *fieldName);
	// Returns the value of a header field within the Nth alternative body's MIME
	// sub-part.
	const uint16_t *altHeaderField(int index, const uint16_t *fieldName);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkEmailU *GetAttachedMessage(int index);
	...
	bool GetAttachedMessageAttr(int index, const uint16_t *fieldName, const uint16_t *attrName, CkString &outStr);
	// Returns a header field attribute value for the Nth attached (embedded) email.
	// For example, to get the value of the "name" attribute in the Content-Type header
	// for the 1st attached message:
	// Content-Type: message/rfc822; name="md75000024149.eml"
	// then the method arguments should contain the values 0, "Content-Type", "name".
	const uint16_t *getAttachedMessageAttr(int index, const uint16_t *fieldName, const uint16_t *attrName);
	// Returns a header field attribute value for the Nth attached (embedded) email.
	// For example, to get the value of the "name" attribute in the Content-Type header
	// for the 1st attached message:
	// Content-Type: message/rfc822; name="md75000024149.eml"
	// then the method arguments should contain the values 0, "Content-Type", "name".
	const uint16_t *attachedMessageAttr(int index, const uint16_t *fieldName, const uint16_t *attrName);
	...
	// Content-Disposition: attachment
	// 
	const uint16_t *getAttachedMessageFilename(int index);
	// Returns the filename of the Nth attached (embedded) email. The filename is the
	// "filename" attribute of the content-disposition header field found within the
	// Nth message/rfc822 sub-part of the calling email object.
	// 
	...
	// Content-Disposition: attachment
	// 
	const uint16_t *attachedMessageFilename(int index);

	// Returns a header field attribute value from the header field of the Nth
	// attachment.
	bool GetAttachmentAttr(int index, const uint16_t *fieldName, const uint16_t *attrName, CkString &outStr);
	// Returns a header field attribute value from the header field of the Nth
	// attachment.
	const uint16_t *getAttachmentAttr(int index, const uint16_t *fieldName, const uint16_t *attrName);
	// Returns a header field attribute value from the header field of the Nth
	// attachment.
	const uint16_t *attachmentAttr(int index, const uint16_t *fieldName, const uint16_t *attrName);

	// Copies an attachment's binary data into binData. The first attachment is at index
	// 0.
	bool GetAttachmentBd(int index, CkBinDataU &binData);
	...
	// Returns the ContentID header field for the Nth attachment. The first attachment
	// is at index 0.
	const uint16_t *getAttachmentContentID(int index);
	// Returns the ContentID header field for the Nth attachment. The first attachment
	// is at index 0.
	const uint16_t *attachmentContentID(int index);
	...
	// Returns the Content-Type header field for the Nth attachment. Indexing of
	// attachments begins at 0.
	const uint16_t *getAttachmentContentType(int index);
	// Returns the Content-Type header field for the Nth attachment. Indexing of
	// attachments begins at 0.
	const uint16_t *attachmentContentType(int index);
	...
	// Retrieves an attachment's filename.
	const uint16_t *getAttachmentFilename(int index);
	// Retrieves an attachment's filename.
	const uint16_t *attachmentFilename(int index);

	// Returns the value of a header field (by name) of an attachment.
	bool GetAttachmentHeader(int attachIndex, const uint16_t *fieldName, CkString &outFieldValue);
	// Returns the value of a header field (by name) of an attachment.
	const uint16_t *getAttachmentHeader(int attachIndex, const uint16_t *fieldName);
	// Returns the value of a header field (by name) of an attachment.
	const uint16_t *attachmentHeader(int attachIndex, const uint16_t *fieldName);
	...
	bool GetAttachmentString(int index, const uint16_t *charset, CkString &outStr);
	// Retrieves an attachment's data as a String. All CRLF sequences will be
	// translated to single newline characters. The charset indicates how to interpret the
	// bytes of the attachment. For example, if the attachment is a text file using the
	// utf-8 byte representation for characters, pass "utf-8".
	const uint16_t *getAttachmentString(int index, const uint16_t *charset);
	// Retrieves an attachment's data as a String. All CRLF sequences will be
	// translated to single newline characters. The charset indicates how to interpret the
	// bytes of the attachment. For example, if the attachment is a text file using the
	// utf-8 byte representation for characters, pass "utf-8".
	const uint16_t *attachmentString(int index, const uint16_t *charset);

	// Retrieves an attachment's data as a String. All end-of-lines will be translated
	// to CRLF sequences.
	bool GetAttachmentStringCrLf(int index, const uint16_t *charset, CkString &outStrData);
	// Retrieves an attachment's data as a String. All end-of-lines will be translated
	// to CRLF sequences.
	const uint16_t *getAttachmentStringCrLf(int index, const uint16_t *charset);
	// Retrieves an attachment's data as a String. All end-of-lines will be translated
	// to CRLF sequences.
	const uint16_t *attachmentStringCrLf(int index, const uint16_t *charset);

	// Returns a blind carbon-copy recipient's full email address.
	bool GetBcc(int index, CkString &outStr);
	// Returns a blind carbon-copy recipient's full email address.
	const uint16_t *getBcc(int index);
	// Returns a blind carbon-copy recipient's full email address.
	const uint16_t *bcc(int index);

	// Returns the Nth BCC address (only the address part, not the friendly-name part).
	bool GetBccAddr(int index, CkString &outStr);
	// Returns the Nth BCC address (only the address part, not the friendly-name part).
	const uint16_t *getBccAddr(int index);
	// Returns the Nth BCC address (only the address part, not the friendly-name part).
	const uint16_t *bccAddr(int index);

	// Returns the Nth BCC name (only the friendly-name part, not the address part).
	bool GetBccName(int index, CkString &outStr);
	// Returns the Nth BCC name (only the friendly-name part, not the address part).
	const uint16_t *getBccName(int index);
	// Returns the Nth BCC name (only the friendly-name part, not the address part).
	const uint16_t *bccName(int index);

	// Returns a carbon-copy recipient's full email address.
	bool GetCC(int index, CkString &outStr);
	// Returns a carbon-copy recipient's full email address.
	const uint16_t *getCC(int index);
	// Returns a carbon-copy recipient's full email address.
	const uint16_t *cC(int index);

	// Returns the Nth CC address (only the address part, not the friendly-name part).
	bool GetCcAddr(int index, CkString &outStr);
	// Returns the Nth CC address (only the address part, not the friendly-name part).
	const uint16_t *getCcAddr(int index);
	// Returns the Nth CC address (only the address part, not the friendly-name part).
	const uint16_t *ccAddr(int index);

	// Returns the Nth CC name (only the friendly-name part, not the address part).
	bool GetCcName(int index, CkString &outStr);
	// Returns the Nth CC name (only the friendly-name part, not the address part).
	const uint16_t *getCcName(int index);
	// Returns the Nth CC name (only the friendly-name part, not the address part).
	const uint16_t *ccName(int index);
	...
	// 
	bool GetDeliveryStatusInfo(const uint16_t *fieldName, CkString &outFieldValue);
	// If the email is a multipart/report, then it is a delivery status notification.
	// This method can be used to get individual pieces of information from the
	// message/delivery-status part of the email. This method should only be called if
	// the IsMultipartReport method returns true.
	...
	// X-Supplementary-Info: 
	// 
	const uint16_t *getDeliveryStatusInfo(const uint16_t *fieldName);
	// If the email is a multipart/report, then it is a delivery status notification.
	// This method can be used to get individual pieces of information from the
	// message/delivery-status part of the email. This method should only be called if
	// the IsMultipartReport method returns true.
	...
	// X-Supplementary-Info: 
	// 
	const uint16_t *deliveryStatusInfo(const uint16_t *fieldName);
	...
	// the IsMultipartReport method returns true.
	// The caller is responsible for deleting the object returned by this method.
	CkStringArrayU *GetDsnFinalRecipients(void);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkCertU *GetEncryptCert(void);

	// Returns the certificate associated with a received encrypted email.
	// The caller is responsible for deleting the object returned by this method.
	CkCertU *GetEncryptedByCert(void);

	// Reads a file and returns the contents as a String. This is here purely for
	// convenience.
	bool GetFileContent(const uint16_t *path, CkByteData &outData);

	// Returns the value of a header field.
	bool GetHeaderField(const uint16_t *fieldName, CkString &outStrFieldData);
	// Returns the value of a header field.
	...
	// The 1st header field is at index 0. (All Chilkat indexing is 0-based.)
	// 
	const uint16_t *getHeaderFieldName(int index);
	// Return the name of the Nth header field. The NumHeaderFields() method can be
	// used to get the number of header fields. The GetHeaderField() method can be used
	// to get the value of the field given the field name.
	// 
	// The 1st header field is at index 0. (All Chilkat indexing is 0-based.)
	// 
	const uint16_t *headerFieldName(int index);
	...
	// NumHeaderFields-1. (All Chilkat indexing is 0-based.)
	// 
	const uint16_t *getHeaderFieldValue(int index);
	// Returns the value of the Nth header field. (Indexing begins at 0) The number of
	// header fields can be obtained from the NumHeaderFields property.
	// 
	// The 1st header field is at index 0, the last header field is at index
	// NumHeaderFields-1. (All Chilkat indexing is 0-based.)
	// 
	const uint16_t *headerFieldValue(int index);

	// Returns the body having the "text/html" content type.
	bool GetHtmlBody(CkString &outStrBody);
	// Returns the body having the "text/html" content type.
	const uint16_t *getHtmlBody(void);
	// Returns the body having the "text/html" content type.
	const uint16_t *htmlBody(void);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkStringArrayU *GetLinkedDomains(void);
	...
	// The 1st header field is at index 0. (All Chilkat indexing is 0-based.)
	// 
	bool GetMbHeaderField(const uint16_t *fieldName, const uint16_t *charset, CkByteData &outBytes);
	...
	// the HTML.
	bool GetMbHtmlBody(const uint16_t *charset, CkByteData &outData);
	...
	bool GetMbPlainTextBody(const uint16_t *charset, CkByteData &outData);
	...
	// Return the email as MIME text containing the email header, body (or bodies),
	// related items (if any), and all attachments
	const uint16_t *getMime(void);
	// Return the email as MIME text containing the email header, body (or bodies),
	// related items (if any), and all attachments
	const uint16_t *mime(void);
	...
	bool GetMimeBd(CkBinDataU &bindat);
	...
	bool GetMimeSb(CkStringBuilderU &sb);
	...
	// 
	bool GetNthBinaryPartOfType(int index, const uint16_t *contentType, bool inlineOnly, bool excludeAttachments, CkByteData &outBytes);
	...
	// those parts that are the attachments will (of course) be missing.
	// 
	bool GetNthTextPartOfType(int index, const uint16_t *contentType, bool inlineOnly, bool excludeAttachments, CkString &outStr);
	// Returns the text of the Nth MIME sub-part having a specified content type (such
	// as "text/plain". Indexing begins at 0. Call GetNumPartsOfType to find out how
	// many MIME sub-parts exist for any given content type. If inlineOnly is true, then
	// only MIME sub-parts having a content-disposition of "inline" are included. If
	...
	// those parts that are the attachments will (of course) be missing.
	// 
	const uint16_t *getNthTextPartOfType(int index, const uint16_t *contentType, bool inlineOnly, bool excludeAttachments);
	// Returns the text of the Nth MIME sub-part having a specified content type (such
	// as "text/plain". Indexing begins at 0. Call GetNumPartsOfType to find out how
	// many MIME sub-parts exist for any given content type. If inlineOnly is true, then
	// only MIME sub-parts having a content-disposition of "inline" are included. If
	...
	// those parts that are the attachments will (of course) be missing.
	// 
	const uint16_t *nthTextPartOfType(int index, const uint16_t *contentType, bool inlineOnly, bool excludeAttachments);
	...
	// those parts that are the attachments will (of course) be missing.
	// 
	int GetNumPartsOfType(const uint16_t *contentType, bool inlineOnly, bool excludeAttachments);

	// Returns the email body having the "text/plain" content type.
	bool GetPlainTextBody(CkString &outStrBody);
	// Returns the email body having the "text/plain" content type.
	const uint16_t *getPlainTextBody(void);
	// Returns the email body having the "text/plain" content type.
	const uint16_t *plainTextBody(void);

	// Returns a header field attribute value from the header field of the Nth related
	// item.
	bool GetRelatedAttr(int index, const uint16_t *fieldName, const uint16_t *attrName, CkString &outStr);
	// Returns a header field attribute value from the header field of the Nth related
	// item.
	const uint16_t *getRelatedAttr(int index, const uint16_t *fieldName, const uint16_t *attrName);
	// Returns a header field attribute value from the header field of the Nth related
	// item.
	const uint16_t *relatedAttr(int index, const uint16_t *fieldName, const uint16_t *attrName);
	...
	// Returns the content ID of a related item contained with the email. Related items
	// are typically images and style-sheets embedded within HTML emails.
	const uint16_t *getRelatedContentID(int index);
	// Returns the content ID of a related item contained with the email. Related items
	// are typically images and style-sheets embedded within HTML emails.
	const uint16_t *relatedContentID(int index);
	...
	// Returns the Content-Location of a related item contained with the email. Related
	// items are typically images and style-sheets embedded within HTML emails.
	const uint16_t *getRelatedContentLocation(int index);
	// Returns the Content-Location of a related item contained with the email. Related
	// items are typically images and style-sheets embedded within HTML emails.
	const uint16_t *relatedContentLocation(int index);
	...
	// Returns the content-type of the Nth related content item in an email message.
	const uint16_t *getRelatedContentType(int index);
	// Returns the content-type of the Nth related content item in an email message.
	const uint16_t *relatedContentType(int index);
	...
	// Returns the filename of a related item contained with the email. Related items
	// are typically images and style-sheets embedded within HTML emails.
	const uint16_t *getRelatedFilename(int index);
	// Returns the filename of a related item contained with the email. Related items
	// are typically images and style-sheets embedded within HTML emails.
	const uint16_t *relatedFilename(int index);
	...
	// emails.
	bool GetRelatedString(int index, const uint16_t *charset, CkString &outStrData);
	// Returns the text with CR line-endings of a related item contained with the
	// email. Related items are typically images and style-sheets embedded within HTML
	// emails.
	const uint16_t *getRelatedString(int index, const uint16_t *charset);
	// Returns the text with CR line-endings of a related item contained with the
	// email. Related items are typically images and style-sheets embedded within HTML
	// emails.
	const uint16_t *relatedString(int index, const uint16_t *charset);

	// Returns the text with CRLF line-endings of a related item contained with the
	// email. Related items are typically images and style-sheets embedded within HTML
	// emails.
	bool GetRelatedStringCrLf(int index, const uint16_t *charset, CkString &outStr);
	// Returns the text with CRLF line-endings of a related item contained with the
	// email. Related items are typically images and style-sheets embedded within HTML
	// emails.
	const uint16_t *getRelatedStringCrLf(int index, const uint16_t *charset);
	// Returns the text with CRLF line-endings of a related item contained with the
	// email. Related items are typically images and style-sheets embedded within HTML
	// emails.
	const uint16_t *relatedStringCrLf(int index, const uint16_t *charset);
	...
	// Returns a replacement pattern previously defined for mail-merge operations.
	const uint16_t *getReplacePattern(int index);
	// Returns a replacement pattern previously defined for mail-merge operations.
	const uint16_t *replacePattern(int index);
	...
	// Returns a replacement string for a previously defined pattern/replacement string
	// pair. (This is a mail-merge feature.)
	const uint16_t *getReplaceString(int index);
	// Returns a replacement string for a previously defined pattern/replacement string
	// pair. (This is a mail-merge feature.)
	const uint16_t *replaceString(int index);

	// Returns a replacement string for a previously defined pattern/replacement string
	// pair. (This is a mail-merge feature.)
	bool GetReplaceString2(const uint16_t *pattern, CkString &outStr);
	// Returns a replacement string for a previously defined pattern/replacement string
	// pair. (This is a mail-merge feature.)
	const uint16_t *getReplaceString2(const uint16_t *pattern);
	// Returns a replacement string for a previously defined pattern/replacement string
	// pair. (This is a mail-merge feature.)
	const uint16_t *replaceString2(const uint16_t *pattern);
	...
	// It is defined in RFC 3462
	// 
	const uint16_t *getReport(int index);
	// (See the NumReports property.) Returns the body content of the Nth report within
	// a multipart/report email.
	// 
	// Multipart/report is a message type that contains data formatted for a mail
	...
	// character.
	bool HasHeaderMatching(const uint16_t *fieldName, const uint16_t *valuePattern, bool caseSensitive);
	...
	// recipients.
	// 
	bool LoadEml(const uint16_t *mimePath);
	...
	// recipients.
	// 
	bool LoadXml(const uint16_t *xmlPath);
	...
	// Takes a byte array of multibyte (non-Unicode) data and returns a Unicode
	// Q-Encoded string.
	bool QEncodeBytes(CkByteData &inData, const uint16_t *charset, CkString &outEncodedStr);
	// Takes a byte array of multibyte (non-Unicode) data and returns a Unicode
	// Q-Encoded string.
	const uint16_t *qEncodeBytes(CkByteData &inData, const uint16_t *charset);

	// Takes a Unicode string, converts it to the charset specified in the 2nd
	// parameter, Q-Encodes the converted multibyte data, and returns the encoded
	// Unicode string.
	bool QEncodeString(const uint16_t *str, const uint16_t *charset, CkString &outEncodedStr);
	// Takes a Unicode string, converts it to the charset specified in the 2nd
	// parameter, Q-Encodes the converted multibyte data, and returns the encoded
	// Unicode string.
	const uint16_t *qEncodeString(const uint16_t *str, const uint16_t *charset);
	...
	void RemoveHeaderField(const uint16_t *fieldName);
	...
	// 
	bool SaveAllAttachments(const uint16_t *dirPath);
	...
	bool SaveAttachedFile(int index, const uint16_t *dirPath);

	// Convert this email object to EML and save it to a file.
	bool SaveEml(const uint16_t *emlFilePath);
	...
	// existing files are allowed to be overwritten.
	bool SaveRelatedItem(int index, const uint16_t *dirPath);
	...
	bool SetAttachmentCharset(int index, const uint16_t *charset);
	...
	bool SetAttachmentDisposition(int index, const uint16_t *disposition);

	// Renames a email attachment's filename. The 1st attachment is at index 0.
	bool SetAttachmentFilename(int index, const uint16_t *filename);
	...
	bool SetBinaryBody(CkByteData &byteData, const uint16_t *contentType, const uint16_t *disposition, const uint16_t *filename);
	...
	// unmodified.
	void SetEdifactBody(const uint16_t *message, const uint16_t *name, const uint16_t *filename, const uint16_t *charset);
	...
	bool SetEncryptCert(CkCertU &cert);
	...
	// recipients.
	// 
	bool SetFromMimeBd(CkBinDataU &bindat);
	...
	// "iso-8859-1", etc.).
	// 
	bool SetFromMimeBytes2(CkByteData &mimeBytes, const uint16_t *charset);
	...
	// recipients.
	// 
	bool SetFromMimeSb(CkStringBuilderU &sb);
	...
	// recipients.
	// 
	bool SetFromMimeText(const uint16_t *mimeText);
	...
	// recipients.
	// 
	bool SetFromXmlText(const uint16_t *xmlStr);

	// Sets the HTML body of an email.
	void SetHtmlBody(const uint16_t *html);
	...
	bool SetMbHtmlBody(const uint16_t *charset, CkByteData &inData);
	...
	bool SetMbPlainTextBody(const uint16_t *charset, CkByteData &inData);

	// Sets the filename for a related item within the email.
	bool SetRelatedFilename(int index, const uint16_t *path);
	...
	// 
	bool SetReplacePattern(const uint16_t *pattern, const uint16_t *replaceString);
	...
	bool SetSigningCert2(CkCertU &cert, CkPrivateKeyU &key);
	...
	void SetTextBody(const uint16_t *bodyText, const uint16_t *contentType);

	// True if the caller email has a UIDL that equals the email passed in the
	// argument.
	bool UidlEquals(CkEmailU &e);
	...
	// 
	bool UnpackHtml(const uint16_t *unpackDir, const uint16_t *htmlFilename, const uint16_t *partsSubdir);
	...
	bool ZipAttachments(const uint16_t *zipFilename);
