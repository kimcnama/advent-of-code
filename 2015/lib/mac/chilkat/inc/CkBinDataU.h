...
class CK_VISIBLE_PUBLIC CkBinDataU  : public CkUtf16Base
{
	...
	int FindString(const uint16_t *str, int startIdx, const uint16_t *charset);
	...
	bool GetTextChunk(int startIdx, int numBytes, const uint16_t *charset, CkString &outStr);
	// Returns numBytes bytes starting at startIdx. The bytes are interpreted according to charset
	// (for example, "utf-8", "ansi", "utf-16", "windows-1252", etc.)
	const uint16_t *getTextChunk(int startIdx, int numBytes, const uint16_t *charset);
	// Returns numBytes bytes starting at startIdx. The bytes are interpreted according to charset
	// (for example, "utf-8", "ansi", "utf-16", "windows-1252", etc.)
	const uint16_t *textChunk(int startIdx, int numBytes, const uint16_t *charset);
