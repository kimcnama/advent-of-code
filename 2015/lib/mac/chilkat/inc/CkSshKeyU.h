// CkSshKeyU.h: interface for the CkSshKeyU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkSshKeyU
class CK_VISIBLE_PUBLIC CkSshKeyU  : public CkUtf16Base
{
	...
	CkSshKeyU(const CkSshKeyU &);
	CkSshKeyU &operator=(const CkSshKeyU &);

    public:
	CkSshKeyU(void);
	virtual ~CkSshKeyU(void);

	

	static CkSshKeyU *createNew(void);
	...
	// -----END RSA PRIVATE KEY-----
	bool FromOpenSshPrivateKey(const uint16_t *keyStr);
	...
	bool FromOpenSshPublicKey(const uint16_t *keyStr);
	...
	bool FromPuttyPrivateKey(const uint16_t *keyStr);
	...
	// zg==
	// ---- END SSH2 PUBLIC KEY ----
	bool FromRfc4716PublicKey(const uint16_t *keyStr);
	...
	// 
	bool GenerateEcdsaKey(const uint16_t *curveName);
	...
	// Generates a fingerpring for an SSH key. A sample fingerprint looks like this:
	// ssh-dss 2048 d0:5f:f7:d6:49:60:7b:50:19:f4:41:59:d4:1f:61:7a
	const uint16_t *genFingerprint(void);

	// Convenience method for loading an entire text file into an in-memory string.
	bool LoadText(const uint16_t *filename, CkString &outStr);
	// Convenience method for loading an entire text file into an in-memory string.
	const uint16_t *loadText(const uint16_t *filename);

	// Convenience method for saving the contents of a string variable to a file.
	bool SaveText(const uint16_t *strToSave, const uint16_t *filename);
	...
	// YIsv2DOWk9HHr0TSG7yu/1hiMf3ol0ofpTEQpXyM8qjVuqCeqsMeJ9I=
	// -----END RSA PRIVATE KEY-----
	const uint16_t *toOpenSshPrivateKey(bool bEncrypt);
	...
	// VJL/pL3tMQrQFqHAg4o94q6M5Y6NvnsSoyZ3gs5bnuyH/wk2oXdlNhRVx1DMYBSdeWRlgjLYUBEKDABs+1N/9nB
	// IZDEUQYvVA71Fawp4cqizg== 
	const uint16_t *toOpenSshPublicKey(void);
	...
	// k9HHr0TSG7yu/1hiMf3ol0ofpTEQpXyM8qjVuqCeqsMeJ9I=
	// Private-MAC: 3BE4CAA1B2AE19C3E6841639BD7275019CF961F1
	const uint16_t *toPuttyPrivateKey(bool bEncrypt);
	...
	// zg==
	// ---- END SSH2 PUBLIC KEY ----
	const uint16_t *toRfc4716PublicKey(void);
