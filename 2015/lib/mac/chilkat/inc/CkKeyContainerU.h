// CkKeyContainerU.h: interface for the CkKeyContainerU class.
//
//////////////////////////////////////////////////////////////////////

// This header is generated for Chilkat 9.5.0.89

#ifndef _CkKeyContainerU_H
#define _CkKeyContainerU_H
...
// CLASS: CkKeyContainerU
class CK_VISIBLE_PUBLIC CkKeyContainerU  : public CkUtf16Base
{
	...
	CkKeyContainerU(const CkKeyContainerU &);
	CkKeyContainerU &operator=(const CkKeyContainerU &);

    public:
	CkKeyContainerU(void);
	virtual ~CkKeyContainerU(void);

	

	static CkKeyContainerU *createNew(void);
	...
	// The name of the currently open key container.
	const uint16_t *containerName(void);
	...
	bool CreateContainer(const uint16_t *name, bool machineKeyset);
	...
	bool DeleteKey(const uint16_t *keyName, const uint16_t *storageProvider);
	...
	bool ExportKey(const uint16_t *keyName, const uint16_t *storageProvider, bool silentFlag, CkPrivateKeyU &key);
	...
	bool GetKeys(const uint16_t *storageProvider, CkJsonObjectU &json);
	...
	// been previously called with the same keyset (machine or user). The index ranges
	// from 0 to GetNumContainers() - 1.
	const uint16_t *getNthContainerName(bool bMachineKeyset, int index);
	// Returns the Nth key container name. The FetchContainerNames method must have
	// been previously called with the same keyset (machine or user). The index ranges
	// from 0 to GetNumContainers() - 1.
	const uint16_t *nthContainerName(bool bMachineKeyset, int index);
	...
	CkPrivateKeyU *GetPrivateKey(bool bKeyExchangePair);
	...
	// signature pair. A null reference is returned on failure.
	// The caller is responsible for deleting the object returned by this method.
	CkPublicKeyU *GetPublicKey(bool bKeyExchangePair);
	...
	// }
	// 
	bool GetStorageProviders(CkJsonObjectU &json);
	...
	bool ImportKey(const uint16_t *keyName, const uint16_t *storageProvider, bool allowOverwrite, bool allowExport, CkPrivateKeyU &key);
	...
	// key pair.
	bool ImportPrivateKey(CkPrivateKeyU &key, bool bKeyExchangePair);
	...
	bool ImportPublicKey(CkPublicKeyU &key, bool bKeyExchangePair);

	// Opens an existing key container in either the machine or user keyset.
	bool OpenContainer(const uint16_t *name, bool needPrivateKeyAccess, bool machineKeyset);
