// CkPkcs11U.h: interface for the CkPkcs11U class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkPkcs11U
class CK_VISIBLE_PUBLIC CkPkcs11U  : public CkUtf16Base
{
	...
	CkPkcs11U(const CkPkcs11U &);
	CkPkcs11U &operator=(const CkPkcs11U &);

    public:
	CkPkcs11U(void);
	virtual ~CkPkcs11U(void);

	

	static CkPkcs11U *createNew(void);
	...
	// <https://man7.org/linux/man-pages/man3/dlopen.3.html>
	// 
	const uint16_t *sharedLibPath(void);
	// On Windows systems, then should be set to the name of the DLL file (if the DLL
	// is located in C:\Windows\System32), or it can be the full path to the DLL.
	// 
	// On Linux, MacOSX, or other non-Windows systems, this can also be either the full
	...
	// <https://man7.org/linux/man-pages/man3/dlopen.3.html>
	// 
	void put_SharedLibPath(const uint16_t *newVal);
	...
	// below.)
	bool Discover(bool onlyTokensPresent, CkJsonObjectU &json);
	...
	bool GetCert(int index, CkCertU &cert);
	...
	// 
	bool InitPin(const uint16_t *pin);
	...
	// 
	bool InitToken(int slotId, const uint16_t *pin, const uint16_t *tokenLabel);
	...
	// 
	bool Login(int userType, const uint16_t *pin);
	...
	bool SetPin(const uint16_t *oldPin, const uint16_t *newPin);
