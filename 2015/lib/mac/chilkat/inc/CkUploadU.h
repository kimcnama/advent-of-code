// CkUploadU.h: interface for the CkUploadU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkUploadU
class CK_VISIBLE_PUBLIC CkUploadU  : public CkClassWithCallbacksU
{
	public:
	bool m_cbOwned;
	...
	CkUploadU(const CkUploadU &);
	CkUploadU &operator=(const CkUploadU &);

    public:
	CkUploadU(void);
	virtual ~CkUploadU(void);

	

	static CkUploadU *createNew(void);
	

	CkUploadU(bool bCallbackOwned);
	static CkUploadU *createNew(bool bCallbackOwned);
	...
	// Returns the response body as a string.
	const uint16_t *responseBodyStr(void);
	...
	void AddFileReference(const uint16_t *name, const uint16_t *filename);
	...
	CkTaskU *BlockingUploadAsync(void);
