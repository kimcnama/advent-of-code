// CkScMinidriverU.h: interface for the CkScMinidriverU class.
//
//////////////////////////////////////////////////////////////////////

// This header is generated for Chilkat 9.5.0.89

#ifndef _CkScMinidriverU_H
#define _CkScMinidriverU_H
...
// CLASS: CkScMinidriverU
class CK_VISIBLE_PUBLIC CkScMinidriverU  : public CkUtf16Base
{
	...
	CkScMinidriverU(const CkScMinidriverU &);
	CkScMinidriverU &operator=(const CkScMinidriverU &);

    public:
	CkScMinidriverU(void);
	virtual ~CkScMinidriverU(void);

	

	static CkScMinidriverU *createNew(void);
	...
	// conjunction with the padding scheme. It can be "SHA1", "SHA256", "SHA384", or
	// "SHA512". The default is "SHA256".
	const uint16_t *rsaPaddingHash(void);
	// If an RSA key is used for signing, this is the hash algorithm to used in
	// conjunction with the padding scheme. It can be "SHA1", "SHA256", "SHA384", or
	// "SHA512". The default is "SHA256".
	void put_RsaPaddingHash(const uint16_t *newVal);
	...
	// If an RSA key is used for signing, this is the padding scheme to use. It can be
	// "PKCS" or "PSS". The default is "PSS".
	const uint16_t *rsaPaddingScheme(void);
	// If an RSA key is used for signing, this is the padding scheme to use. It can be
	// "PKCS" or "PSS". The default is "PSS".
	void put_RsaPaddingScheme(const uint16_t *newVal);
	...
	bool AcquireContext(const uint16_t *readerName);
	...
	bool CardDeleteFile(const uint16_t *dirName, const uint16_t *fileName);
	...
	// deleted.
	bool DeleteCert(CkCertU &cert, bool delPrivKey);
	...
	bool EnumFiles(const uint16_t *dirName, CkStringTableU &st);
	...
	// etc.
	// 
	bool FindCert(const uint16_t *certPart, const uint16_t *partValue, CkCertU &cert);
	...
	// 
	bool GenerateKey(int containerIndex, const uint16_t *keySpec, const uint16_t *keyType, int keySize, const uint16_t *pinId);

	// Gets all card properties and returns them in json. See the example below.
	bool GetCardProperties(CkJsonObjectU &json);
	...
	// etc.
	// 
	bool GetCert(int containerIndex, const uint16_t *keySpec, CkCertU &cert);
	...
	bool GetContainerKeys(int containerIndex, CkPublicKeyU &sigKey, CkPublicKeyU &kexKey);
	...
	// linked below.
	bool GetCspContainerMap(CkJsonObjectU &json);
	...
	// The pinId can be "user", or "3" through "7". (It is typically "user".)
	// 
	bool ImportCert(CkCertU &cert, int containerIndex, const uint16_t *keySpec, const uint16_t *pinId);
	...
	// 
	bool ImportKey(int containerIndex, const uint16_t *keySpec, CkPrivateKeyU &privKey, const uint16_t *pinId);
	...
	bool ListCerts(const uint16_t *certPart, CkStringTableU &st);
	...
	// 
	int PinAuthenticate(const uint16_t *pinId, const uint16_t *pin);
	...
	int PinAuthenticateHex(const uint16_t *pinId, const uint16_t *pin);
	...
	// not yet been acquired, the return value is -2.
	// 
	int PinChange(const uint16_t *pinId, const uint16_t *currentPin, const uint16_t *newPin);
	...
	bool PinDeauthenticate(const uint16_t *pinId);
	...
	bool ReadFile(const uint16_t *dirName, const uint16_t *fileName, CkBinDataU &bd);
	...
	// 
	bool SignData(int containerIndex, const uint16_t *keySpec, const uint16_t *hashDataAlg, CkBinDataU &bdData, CkBinDataU &bdSignedData);
	...
	bool WriteFile(const uint16_t *dirName, const uint16_t *fileName, CkBinDataU &bd);
