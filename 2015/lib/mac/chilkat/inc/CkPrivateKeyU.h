...
class CK_VISIBLE_PUBLIC CkPrivateKeyU  : public CkUtf16Base
{
	...
	// 
	bool GetRawHex(CkStringBuilderU &pubKey, CkString &outStr);
	// Returns the private key in raw hex format (lowercase). The public key is written
	// to pubKey.
	// 
	// Ed25519 private and public keys are 32-byte each (64 chars in hex format).
	...
	// multiple parts (modulus, exponent, and more).
	// 
	const uint16_t *getRawHex(CkStringBuilderU &pubKey);
	// Returns the private key in raw hex format (lowercase). The public key is written
	// to pubKey.
	// 
	// Ed25519 private and public keys are 32-byte each (64 chars in hex format).
	...
	// multiple parts (modulus, exponent, and more).
	// 
	const uint16_t *rawHex(CkStringBuilderU &pubKey);
	...
	bool LoadAnyFormatFile(const uint16_t *path, const uint16_t *password);
	...
	bool LoadEd25519(const uint16_t *privKey, const uint16_t *pubKey);
