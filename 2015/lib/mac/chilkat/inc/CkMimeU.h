// CkMimeU.h: interface for the CkMimeU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkMimeU
class CK_VISIBLE_PUBLIC CkMimeU  : public CkUtf16Base
{
	...
	CkMimeU(const CkMimeU &);
	CkMimeU &operator=(const CkMimeU &);

    public:
	CkMimeU(void);
	virtual ~CkMimeU(void);

	

	static CkMimeU *createNew(void);
	...
	// explicitly setting the boundary is usually not necessary.
	// 
	void put_Boundary(const uint16_t *newVal);
	...
	// then the value of the ContentType property is "multipart/mixed".
	// 
	const uint16_t *contentType(void);
	// The MIME content type, such as "text/plain", "text/html", "image/gif",
	// "multipart/alternative", "multipart/mixed", etc.
	// 
	// It is the value of the Content-Type header field, excluding any attributes. For
	...
	// Content-Disposition: attachment; filename="starfish.gif"
	// then the value of the Disposition property is "attachment".
	const uint16_t *disposition(void);
	// The value of the Content-Disposition header field, excluding any attributes. For
	// example, if the Content-Disposition header is this:
	// Content-Disposition: attachment; filename="starfish.gif"
	// then the value of the Disposition property is "attachment".
	void put_Disposition(const uint16_t *newVal);
	...
	// property instead.
	// 
	void put_Micalg(const uint16_t *newVal);
	...
	// Content-Type: image/gif; name="starfish.gif"
	// then the value of the Name property is "starfish.gif".
	const uint16_t *name(void);
	// The value of the "name" attribute of the Content-Type header field. For example,
	// if the Content-Type header is this:
	// Content-Type: image/gif; name="starfish.gif"
	// then the value of the Name property is "starfish.gif".
	void put_Name(const uint16_t *newVal);
	...
	//   boundary="------------ms000908010507020408060303"
	// then the value of the Protocol property is "application/x-pkcs7-signature".
	const uint16_t *protocol(void);
	// The value of the "protocol" attribute of the Content-Type header field. For
	// example, if the Content-Type header is this:
	// Content-Type: multipart/signed; protocol="application/x-pkcs7-signature"; micalg=sha1; 
	//   boundary="------------ms000908010507020408060303"
	// then the value of the Protocol property is "application/x-pkcs7-signature".
	void put_Protocol(const uint16_t *newVal);
	...
	// AddPfxSourceData.
	bool AddDecryptCert(CkCertU &cert);
	...
	// the first sub-part.
	bool AddDetachedSignature(CkCertU &cert);
	...
	// signature.
	bool AddDetachedSignature2(CkCertU &cert, bool transferHeaderFields);
	...
	bool AddDetachedSignaturePk(CkCertU &cert, CkPrivateKeyU &privateKey);
	...
	// original content of the calling object, and the second contains the digital
	// signature.
	bool AddDetachedSignaturePk2(CkCertU &cert, CkPrivateKeyU &privateKey, bool transferHeaderFields);
	...
	bool AddHeaderField(const uint16_t *name, const uint16_t *value);
	...
	// 
	bool AddPfxSourceData(CkByteData &pfxFileData, const uint16_t *pfxPassword);
	...
	bool AppendPartFromFile(const uint16_t *filename);
	...
	//                 _LT_contextSpecific tag="0" constructed="1">
	// ...
	const uint16_t *asnBodyToXml(void);
	...
	// 
	bool ConvertToSigned(CkCertU &cert);
	...
	bool ConvertToSignedPk(CkCertU &cert, CkPrivateKeyU &privateKey);
	...
	bool Decrypt2(CkCertU &cert, CkPrivateKeyU &privateKey);

	// Decrypts PKCS7 encrypted MIME (also known as S/MIME) using a specific
	// certificate.
	bool DecryptUsingCert(CkCertU &cert);
	...
	// database.
	bool DecryptUsingPfxData(CkByteData &pfxData, const uint16_t *password);
	...
	// PKCS12 format.)
	bool DecryptUsingPfxFile(const uint16_t *pfxFilePath, const uint16_t *pfxPassword);
	...
	CkStringArrayU *ExtractPartsToFiles(const uint16_t *dirPath);
	...
	bool GetBodyBd(CkBinDataU &binDat);
	...
	// converted from its encoding type, such as base64 or quoted-printable, before
	// being returned.
	const uint16_t *getBodyDecoded(void);
	// Returns the body of the MIME message as a string. The body is automatically
	// converted from its encoding type, such as base64 or quoted-printable, before
	// being returned.
	const uint16_t *bodyDecoded(void);
	...
	bool GetHeaderFieldAttribute(const uint16_t *name, const uint16_t *attrName, CkString &outStr);
	// Parses a MIME header field and returns the value of an attribute. MIME header
	// fields w/ attributes are formatted like this:
	// Header-Name:  value;  attrName1="value1"; attrName2="value2"; ....  attrNameN="valueN"
	// Semi-colons separate attribute name=value pairs. The Content-Type header field
	...
	// In the above example, to access the value of the "protocol" attribute, call
	// GetHeaderFieldAttribute("Content-Type", "protocol");
	const uint16_t *getHeaderFieldAttribute(const uint16_t *name, const uint16_t *attrName);
	// Parses a MIME header field and returns the value of an attribute. MIME header
	// fields w/ attributes are formatted like this:
	// Header-Name:  value;  attrName1="value1"; attrName2="value2"; ....  attrNameN="valueN"
	// Semi-colons separate attribute name=value pairs. The Content-Type header field
	...
	// In the above example, to access the value of the "protocol" attribute, call
	// GetHeaderFieldAttribute("Content-Type", "protocol");
	const uint16_t *headerFieldAttribute(const uint16_t *name, const uint16_t *attrName);
	...
	bool GetStructure(const uint16_t *fmt, CkString &outStr);
	// Returns a string summarizing the MIME structure. The output format is specified
	// by fmt and can be "text" or "xml".
	const uint16_t *getStructure(const uint16_t *fmt);
	// Returns a string summarizing the MIME structure. The output format is specified
	// by fmt and can be "text" or "xml".
	const uint16_t *structure(const uint16_t *fmt);
	...
	// from a string.
	bool LoadMime(const uint16_t *mimeMsg);

	// Discards the current contents of the MIME object and loads a new MIME message
	// from a BinData object.
	bool LoadMimeBd(CkBinDataU &bindat);
	...
	// Discards the current contents of the MIME object and loads a new MIME message
	// from a file.
	bool LoadMimeFile(const uint16_t *fileName);

	// Discards the current contents of the MIME object and loads a new MIME message
	// from a StringBuilder.
	bool LoadMimeSb(CkStringBuilderU &sb);
	...
	bool NewMessageRfc822(CkMimeU &mimeObject);
	...
	// is removed.
	void RemoveHeaderField(const uint16_t *fieldName, bool bAllOccurrences);
	...
	void SetBody(const uint16_t *str);
	...
	bool SetBodyFromEncoded(const uint16_t *encoding, const uint16_t *str);
	...
	// 
	bool SetBodyFromFile(const uint16_t *fileName);
	...
	// 
	bool SetBodyFromHtml(const uint16_t *str);
	...
	// To set the MIME body with no intentional side-effects, use SetBody instead.
	// 
	bool SetBodyFromPlainText(const uint16_t *str);
	...
	// To set the MIME body with no intentional side-effects, use SetBody instead.
	// 
	bool SetBodyFromXml(const uint16_t *str);
	...
	// the header field.
	bool SetHeaderField(const uint16_t *name, const uint16_t *value);
	...
	// 
	// Soci%E9t%E9
	void UrlEncodeBody(const uint16_t *charset);
