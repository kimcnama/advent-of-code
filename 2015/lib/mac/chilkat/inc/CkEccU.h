...
class CK_VISIBLE_PUBLIC CkEccU  : public CkUtf16Base
{
	...
	bool SignBd(CkBinDataU &bdData, const uint16_t *hashAlg, const uint16_t *encoding, CkPrivateKeyU &privKey, CkPrngU &prng, CkString &outStr);
	// This method is the same as SignHashENC, except the actual data to be signed and
	// the name of the hash algorithm is passed in. The following hash algorithms are
	// supported: sha256, sha384, and sha512.
	const uint16_t *signBd(CkBinDataU &bdData, const uint16_t *hashAlg, const uint16_t *encoding, CkPrivateKeyU &privKey, CkPrngU &prng);
	...
	int VerifyBd(CkBinDataU &bdData, const uint16_t *hashAlg, const uint16_t *encodedSig, const uint16_t *encoding, CkPublicKeyU &pubkey);
