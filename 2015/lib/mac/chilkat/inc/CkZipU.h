// CkZipU.h: interface for the CkZipU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkZipU
class CK_VISIBLE_PUBLIC CkZipU  : public CkClassWithCallbacksU
{
	public:
	bool m_cbOwned;
	...
	static CkZipU *createNew(void);
	

	CkZipU(bool bCallbackOwned);
	static CkZipU *createNew(bool bCallbackOwned);
	...
	CkZipProgressU *get_EventCallbackObject(void) const;
	void put_EventCallbackObject(CkZipProgressU *progress);
	...
	// path that is saved in the .zip would be "myAppDir/". (The value of the
	// AppendFromDir property does not become part of the file path saved in the .zip.)
	const uint16_t *appendFromDir(void);
	// When files are added to a Zip archive, they are appended from this directory.
	// For example, to add all the files under c:/abc/123/myAppDir, this property could
	// be set to "c:/abc/123", and "myAppDir/*" would be passed to AppendFiles. The
	// path that is saved in the .zip would be "myAppDir/". (The value of the
	// AppendFromDir property does not become part of the file path saved in the .zip.)
	void put_AppendFromDir(const uint16_t *newVal);
	...
	// executable contained within the to-be-created EXE that will automatically be run
	// after extraction. (This is typically something like "setup.exe")
	const uint16_t *autoRun(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Optional
	// when creating Windows-based self-extracting EXEs. This is the name of an
	// executable contained within the to-be-created EXE that will automatically be run
	// after extraction. (This is typically something like "setup.exe")
	void put_AutoRun(const uint16_t *newVal);
#endif
	...
	// (Relevant only when running on a Microsoft Windows operating system.) Command
	// line parameters that get passed to the AutoRun executable.
	const uint16_t *autoRunParams(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Command
	// line parameters that get passed to the AutoRun executable.
	void put_AutoRunParams(const uint16_t *newVal);
#endif
	...
	// password.
	// 
	const uint16_t *decryptPassword(void);
	// When opening a password-protected or AES encrypted Zip, this is the password to
	// be used for decryption. Encrypted Zips may be opened without setting a password,
	// but the contents cannot be unzipped without setting this password.
	// 
	...
	// password.
	// 
	void put_DecryptPassword(const uint16_t *newVal);
	...
	// password.
	// 
	const uint16_t *encryptPassword(void);
	// The password used when writing a password-protected or strong-encrytped Zip.
	// 
	// Note:The SetPassword method has the effect of setting both this property as well
	// as the DecryptPassword property. The SetPassword method should no longer be
	...
	// password.
	// 
	void put_EncryptPassword(const uint16_t *newVal);
	...
	// the default unzip directory path to appear in the user-interface dialog box when
	// the Windows-based self-extracting EXE is run.
	const uint16_t *exeDefaultDir(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Specifies
	// the default unzip directory path to appear in the user-interface dialog box when
	// the Windows-based self-extracting EXE is run.
	void put_ExeDefaultDir(const uint16_t *newVal);
#endif
	...
	// application if an ExeIconFile is to be used.)
	// 
	const uint16_t *exeIconFile(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Applies to
	// creating self-extracting EXEs. This property can be set to a pre-existing icon
	// filename (.ico) that will be embedded within the to-be-created EXE and set as
	// its default icon.
	...
	// application if an ExeIconFile is to be used.)
	// 
	void put_ExeIconFile(const uint16_t *newVal);
#endif
	...
	// creating Windows-based self-extracting EXEs. Sets the title of the main
	// user-interface dialog that appears when the self-extracting EXE runs.
	const uint16_t *exeTitle(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Applies to
	// creating Windows-based self-extracting EXEs. Sets the title of the main
	// user-interface dialog that appears when the self-extracting EXE runs.
	void put_ExeTitle(const uint16_t *newVal);
#endif
	...
	// creating MS Windows-based self-extracting EXEs. Sets the unzipping caption of
	// the main user-interface dialog that appears when the self-extracting EXE runs.
	const uint16_t *exeUnzipCaption(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Applies to
	// creating MS Windows-based self-extracting EXEs. Sets the unzipping caption of
	// the main user-interface dialog that appears when the self-extracting EXE runs.
	void put_ExeUnzipCaption(const uint16_t *newVal);
#endif
	...
	// ExeFinishNotifier = false
	// 
	const uint16_t *exeUnzipDir(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Applies to
	// creating MS Windows self-extracting EXEs. Stores a pre-defined unzip directory
	// within the self-extracting EXE so that it automatically unzips to this directory
	// without user-intervention.
	...
	// ExeFinishNotifier = false
	// 
	void put_ExeUnzipDir(const uint16_t *newVal);
#endif
	...
	// {logFilePath}" option to create a log with information for debugging.
	// 
	const uint16_t *exeXmlConfig(void);
	// (Relevant only when running on a Microsoft Windows operating system.) Allows for
	// an XML config document to be used to specify all possible options for
	// self-extracting EXEs. This property is a string containing the XML config
	// document.
	...
	// {logFilePath}" option to create a log with information for debugging.
	// 
	void put_ExeXmlConfig(const uint16_t *newVal);
#endif
	...
	// value is "ansi". Other possible choices are cp850, cp437, or any of the code
	// pages listed at the link below.
	const uint16_t *pwdProtCharset(void);
	// For older password-protected Zip archives (Zip 2.0 encryption), specifies the
	// charset used for the binary representation of the decrypt password. The default
	// value is "ansi". Other possible choices are cp850, cp437, or any of the code
	// pages listed at the link below.
	void put_PwdProtCharset(const uint16_t *newVal);
	...
	// ASP.NET, the default value of TempDir is set to the directory where the .zip is
	// being written. Set this property to override the default.
	const uint16_t *tempDir(void);
	// The temporary directory to use when unzipping files. When running in ASP or
	// ASP.NET, the default value of TempDir is set to the directory where the .zip is
	// being written. Set this property to override the default.
	void put_TempDir(const uint16_t *newVal);
	...
	// indicate an obvious choice for the appropriate compression algorithm, then the
	// ZipxDefaultAlg is used.
	const uint16_t *zipxDefaultAlg(void);
	// The default compression algorithm to be used when creating ZIPX archives. The
	// default value is "deflate". Other possible values are "ppmd", "lzma", "bzip2"
	// and "deflate64". When writing a ZIPX archive, if the file extension does not
	// indicate an obvious choice for the appropriate compression algorithm, then the
	// ZipxDefaultAlg is used.
	void put_ZipxDefaultAlg(const uint16_t *newVal);
	...
	// 
	bool AddEmbedded(const uint16_t *exeFilename, const uint16_t *resourceName, const uint16_t *zipFilename);
#endif
	...
	// 
	void AddNoCompressExtension(const uint16_t *fileExtension);
	...
	CkZipEntryU *AppendBase64(const uint16_t *fileName, const uint16_t *encodedCompressedData);

	// Appends the contents of byteData as a new entry to this zip object. The zip entry
	// object containing the data is returned.
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendBd(const uint16_t *pathInZip, CkBinDataU &byteData);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendCompressed(const uint16_t *filename, CkByteData &inData);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendData(const uint16_t *fileName, CkByteData &inData);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendDataEncoded(const uint16_t *filename, const uint16_t *encoding, const uint16_t *data);
	...
	// either the WriteZip or WriteZipAndClose method would need to be called.
	// 
	bool AppendFiles(const uint16_t *filePattern, bool recurse);

	// Creates an asynchronous task to call the AppendFiles method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AppendFilesAsync(const uint16_t *filePattern, bool recurse);
	...
	// 
	bool AppendFilesEx(const uint16_t *filePattern, bool recurse, bool saveExtraPath, bool archiveOnly, bool includeHidden, bool includeSystem);

	// Creates an asynchronous task to call the AppendFilesEx method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AppendFilesExAsync(const uint16_t *filePattern, bool recurse, bool saveExtraPath, bool archiveOnly, bool includeHidden, bool includeSystem);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendHex(const uint16_t *fileName, const uint16_t *encodedCompressedData);
	...
	// either the WriteZip or WriteZipAndClose method would need to be called.
	// 
	bool AppendMultiple(CkStringArrayU &fileSpecs, bool recurse);

	// Creates an asynchronous task to call the AppendMultiple method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AppendMultipleAsync(CkStringArrayU &fileSpecs, bool recurse);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendNew(const uint16_t *fileName);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendNewDir(const uint16_t *dirName);
	...
	// no effect.
	bool AppendOneFileOrDir(const uint16_t *fileOrDirPath, bool saveExtraPath);

	// Creates an asynchronous task to call the AppendOneFileOrDir method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AppendOneFileOrDirAsync(const uint16_t *fileOrDirPath, bool saveExtraPath);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendString(const uint16_t *internalZipFilepath, const uint16_t *textData);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *AppendString2(const uint16_t *internalZipFilepath, const uint16_t *textData, const uint16_t *charset);

	// Adds the contents of another existing Zip file to this Zip object.
	bool AppendZip(const uint16_t *zipFileName);
	...
	bool DeleteEntry(CkZipEntryU &entry);
	...
	void ExcludeDir(const uint16_t *dirName);
	...
	bool ExtractExe(const uint16_t *exePath, const uint16_t *dirPath);
#endif
	...
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ExtractExeAsync(const uint16_t *exePath, const uint16_t *dirPath);
#endif
	...
	bool ExtractMatching(const uint16_t *dirPath, const uint16_t *pattern);
	...
	// updated.
	bool ExtractNewer(const uint16_t *dirPath);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *FirstMatchingEntry(const uint16_t *pattern);

	// Return the contents of the Zip file directory in an XML formatted string
	bool GetDirectoryAsXML(CkString &outXml);
	// Return the contents of the Zip file directory in an XML formatted string
	const uint16_t *getDirectoryAsXML(void);
	// Return the contents of the Zip file directory in an XML formatted string
	const uint16_t *directoryAsXML(void);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *GetEntryByName(const uint16_t *entryName);

	// Returns the current collection of exclusion patterns that have been set by
	// SetExclusions.
	// The caller is responsible for deleting the object returned by this method.
	CkStringArrayU *GetExclusions(void);
	...
	bool GetExeConfigParam(const uint16_t *name, CkString &outStr);
	// (Relevant only when running on a Microsoft Windows operating system.) Gets the
	// value of an EXE config param as described in the ExeXmlConfig property.
	const uint16_t *getExeConfigParam(const uint16_t *name);
	// (Relevant only when running on a Microsoft Windows operating system.) Gets the
	// value of an EXE config param as described in the ExeXmlConfig property.
	const uint16_t *exeConfigParam(const uint16_t *name);
#endif
	...
	// The caller is responsible for deleting the object returned by this method.
	CkZipEntryU *InsertNew(const uint16_t *fileName, int beforeIndex);
	...
	bool IsNoCompressExtension(const uint16_t *fileExtension);

	// Return True if a Zip file is password protected
	bool IsPasswordProtected(const uint16_t *zipFilename);
	...
	bool NewZip(const uint16_t *zipFilePath);
	...
	bool OpenMyEmbedded(const uint16_t *resourceName);
#endif
	...
	// where 4 indicates WinZip compatible AES encryption.
	// 
	bool OpenZip(const uint16_t *zipPath);

	// Creates an asynchronous task to call the OpenZip method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *OpenZipAsync(const uint16_t *zipPath);
	...
	void RemoveNoCompressExtension(const uint16_t *fileExtension);
	...
	bool ReplaceEmbedded(const uint16_t *exeFilename, const uint16_t *resourceName, const uint16_t *zipFilename);
#endif
	...
	void SetExclusions(CkStringArrayU &excludePatterns);
	...
	// 
	void SetExeConfigParam(const uint16_t *paramName, const uint16_t *paramValue);
#endif
	...
	// 
	void SetPassword(const uint16_t *password);
	...
	int Unzip(const uint16_t *dirPath);

	// Creates an asynchronous task to call the Unzip method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *UnzipAsync(const uint16_t *dirPath);
	...
	// unzipped.
	int UnzipInto(const uint16_t *dirPath);

	// Creates an asynchronous task to call the UnzipInto method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *UnzipIntoAsync(const uint16_t *dirPath);
	...
	// 
	bool WriteExe(const uint16_t *exeFilename);
#endif
	...
	bool WriteExe2(const uint16_t *exePath, const uint16_t *destExePath, bool bAesEncrypt, int keyLength, const uint16_t *password);
#endif
	...
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *WriteToMemoryAsync(void);
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *WriteZipAndCloseAsync(void);
