// CkJweU.h: interface for the CkJweU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkJweU
class CK_VISIBLE_PUBLIC CkJweU  : public CkUtf16Base
{
	...
	//     A128CBC-HS256, A192CBC-HS384, A256CBC-HS512, A128GCM, A192GCM, A256GCM
	// 
	bool Decrypt(int index, const uint16_t *charset, CkString &outStr);
	// Decrypts a JWE and returns the original (decrypted) string content. The byte
	// representation of the decrypted bytes is indicated by charset (such as "utf-8").
	// (The charset tells Chilkat how to intepret the decrypted bytes as characters.)
	// 
	...
	//     A128CBC-HS256, A192CBC-HS384, A256CBC-HS512, A128GCM, A192GCM, A256GCM
	// 
	const uint16_t *decrypt(int index, const uint16_t *charset);

	// Decrypts the loaded JWE and appends the decrypted bytes to the contents of bd.
	// The index specifies which recipient key is used for decryption. (Most JWEs have
	// only a single recipent, and thus the index is typically 0.)
	bool DecryptBd(int index, CkBinDataU &bd);
	...
	// only a single recipent, and thus the index is typically 0.)
	// 
	bool DecryptSb(int index, const uint16_t *charset, CkStringBuilderU &contentSb);
	...
	//     A128CBC-HS256, A192CBC-HS384, A256CBC-HS512, A128GCM, A192GCM, A256GCM
	// 
	bool Encrypt(const uint16_t *content, const uint16_t *charset, CkString &outStr);
	// Encrypts string content to produce a JWE. The byte representation of the content is
	// indicated by charset (such as "utf-8").
	// 
	// Supported Algorithms:
	...
	//     A128CBC-HS256, A192CBC-HS384, A256CBC-HS512, A128GCM, A192GCM, A256GCM
	// 
	const uint16_t *encrypt(const uint16_t *content, const uint16_t *charset);
	...
	bool EncryptBd(CkBinDataU &contentBd, CkStringBuilderU &jweSb);
	...
	bool EncryptSb(CkStringBuilderU &contentSb, const uint16_t *charset, CkStringBuilderU &jweSb);
	...
	bool SetPassword(int index, const uint16_t *password);
	...
	bool SetProtectedHeader(CkJsonObjectU &json);
	...
	bool SetRecipientHeader(int index, CkJsonObjectU &json);

	// Sets the JWE Shared Unprotected Header.
	bool SetUnprotectedHeader(CkJsonObjectU &json);
	...
	// 
	bool SetWrappingKey(int index, const uint16_t *encodedKey, const uint16_t *encoding);
