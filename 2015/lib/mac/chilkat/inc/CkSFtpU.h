// CkSFtpU.h: interface for the CkSFtpU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkSFtpU
class CK_VISIBLE_PUBLIC CkSFtpU  : public CkClassWithCallbacksU
{
	public:
	bool m_cbOwned;
	...
	CkSFtpU(const CkSFtpU &);
	CkSFtpU &operator=(const CkSFtpU &);

    public:
	CkSFtpU(void);
	virtual ~CkSFtpU(void);

	

	static CkSFtpU *createNew(void);
	

	CkSFtpU(bool bCallbackOwned);
	static CkSFtpU *createNew(bool bCallbackOwned);
	...
	CkSFtpProgressU *get_EventCallbackObject(void) const;
	void put_EventCallbackObject(CkSFtpProgressU *progress);
	...
	// because some servers are known to disconnect from clients with unknown client
	// identifiers.)
	const uint16_t *clientIdentifier(void);
	// The client-identifier string to be used when connecting to an SSH/SFTP server.
	// Defaults to "SSH-2.0-PuTTY_Release_0.63". (This string is used to mimic PuTTY
	// because some servers are known to disconnect from clients with unknown client
	// identifiers.)
	void put_ClientIdentifier(const uint16_t *newVal);
	...
	// this property contains a descriptive string for the "reason code" as specified
	// in RFC 4253.
	const uint16_t *disconnectReason(void);
	...
	// Otherwise, incoming and outgoing filenames use the charset specified by this
	// property.
	const uint16_t *filenameCharset(void);
	// Automatically set during the InitializeSftp method if the server sends a
	// filename-charset name-value extension. Otherwise, may be set directly to the
	// name of a charset, such as "utf-8", "iso-8859-1", "windows-1252", etc. ("ansi"
	// is also a valid choice.) Incoming filenames are interpreted as utf-8 if no
	...
	// Otherwise, incoming and outgoing filenames use the charset specified by this
	// property.
	void put_FilenameCharset(const uint16_t *newVal);
	...
	// encryption algorithm, then the Connect will fail.
	// 
	const uint16_t *forceCipher(void);
	// Set to one of the following encryption algorithms to force that cipher to be
	// used. By default, the component will automatically choose the first cipher
	// supported by the server in the order listed here: "aes256-ctr", "aes128-ctr",
	// "aes256-cbc", "aes128-cbc", "twofish256-cbc", "twofish128-cbc", "blowfish-cbc",
	...
	// encryption algorithm, then the Connect will fail.
	// 
	void put_ForceCipher(const uint16_t *newVal);
	...
	// secure connection. The default is "DSS". It may be changed to "RSA" if needed.
	// Chilkat recommends not changing this unless a problem warrants the change.
	const uint16_t *hostKeyAlg(void);
	// Indicates the preferred host key algorithm to be used in establishing the SSH
	// secure connection. The default is "DSS". It may be changed to "RSA" if needed.
	// Chilkat recommends not changing this unless a problem warrants the change.
	void put_HostKeyAlg(const uint16_t *newVal);
	...
	// reason for failure. (It contains descriptive text matching the
	// InitializeFailCode property.)
	const uint16_t *initializeFailReason(void);
	...
	// SFTP server implementation. The integer status code is available in the
	// LastStatusCode property.
	const uint16_t *lastStatusMessage(void);
	...
	// example, "*.xml; *.txt; *.csv". If set, the ReadDir method will only return
	// files that match any one of these patterns.
	const uint16_t *readDirMustMatch(void);
	// Can contain a wildcarded list of file patterns separated by semicolons. For
	// example, "*.xml; *.txt; *.csv". If set, the ReadDir method will only return
	// files that match any one of these patterns.
	void put_ReadDirMustMatch(const uint16_t *newVal);
	...
	// example, "*.xml; *.txt; *.csv". If set, the ReadDir method will only return
	// files that do not match any of these patterns.
	const uint16_t *readDirMustNotMatch(void);
	// Can contain a wildcarded list of file patterns separated by semicolons. For
	// example, "*.xml; *.txt; *.csv". If set, the ReadDir method will only return
	// files that do not match any of these patterns.
	void put_ReadDirMustNotMatch(const uint16_t *newVal);
	...
	// establishment. For example, a typical value would be similar to
	// "SSH-2.0-OpenSSH_7.2p2 Ubuntu-4ubuntu2.2".
	const uint16_t *serverIdentifier(void);
	...
	// system has the file open for exclusive access.
	// 
	const uint16_t *syncDirectives(void);
	// A property that can contain a list of comma-separated keywords to control
	// certain aspects of an upload or download synchronization (for the SyncTreeUpload
	// and SyncTreeDownload methods). At this time there is only one possible
	// directive, but others may be added in the future.
	...
	// system has the file open for exclusive access.
	// 
	void put_SyncDirectives(const uint16_t *newVal);
	...
	// for an application to display this message if it is doing keyboard-interactive
	// authentication via the StartKeyboardAuth and ContinueKeyboardAuth methods.
	const uint16_t *userAuthBanner(void);
	// If a user authentication banner message is received during authentication, it
	// will be made available here. An application can check to see if this property
	// contains a banner string after calling StartKeyboardAuth. It is only possible
	// for an application to display this message if it is doing keyboard-interactive
	// authentication via the StartKeyboardAuth and ContinueKeyboardAuth methods.
	void put_UserAuthBanner(const uint16_t *newVal);
	...
	// -1 on error.
	// 
	int AccumulateBytes(const uint16_t *handle, int maxBytes);

	// Creates an asynchronous task to call the AccumulateBytes method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *AccumulateBytesAsync(const uint16_t *handle, int maxBytes);
	...
	bool Add64(const uint16_t *n1, const uint16_t *n2, CkString &outStr);
	// Convenience method for 64-bit addition. Allows for two 64-bit integers to be
	// passed as decimal strings and returns the sum in a decimal sting.
	const uint16_t *add64(const uint16_t *n1, const uint16_t *n2);
	...
	// 
	bool Connect(const uint16_t *domainName, int port);

	// Creates an asynchronous task to call the Connect method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ConnectAsync(const uint16_t *domainName, int port);
	...
	bool ConnectThroughSsh(CkSshU &sshConn, const uint16_t *hostname, int port);

	// Creates an asynchronous task to call the ConnectThroughSsh method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ConnectThroughSshAsync(CkSshU &sshConn, const uint16_t *hostname, int port);
	...
	// time zones.
	// 
	bool CopyFileAttr(const uint16_t *localFilename, const uint16_t *remoteFilename, bool isHandle);

	// Creates an asynchronous task to call the CopyFileAttr method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *CopyFileAttrAsync(const uint16_t *localFilename, const uint16_t *remoteFilename, bool isHandle);

	// Creates a directory on the SFTP server.
	bool CreateDir(const uint16_t *path);

	// Creates an asynchronous task to call the CreateDir method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *CreateDirAsync(const uint16_t *path);
	...
	// BinData.)
	bool DownloadBd(const uint16_t *remoteFilePath, CkBinDataU &binData);

	// Creates an asynchronous task to call the DownloadBd method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DownloadBdAsync(const uint16_t *remoteFilePath, CkBinDataU &binData);
	...
	bool DownloadFile(const uint16_t *handle, const uint16_t *toFilename);

	// Creates an asynchronous task to call the DownloadFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DownloadFileAsync(const uint16_t *handle, const uint16_t *toFilename);
	...
	// 
	bool DownloadFileByName(const uint16_t *remoteFilePath, const uint16_t *localFilePath);

	// Creates an asynchronous task to call the DownloadFileByName method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DownloadFileByNameAsync(const uint16_t *remoteFilePath, const uint16_t *localFilePath);

	// Downloads the contents of a remote file to a StringBuilder object. (Appends to
	// the StringBuilder.)
	bool DownloadSb(const uint16_t *remoteFilePath, const uint16_t *charset, CkStringBuilderU &sb);

	// Creates an asynchronous task to call the DownloadSb method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *DownloadSbAsync(const uint16_t *remoteFilePath, const uint16_t *charset, CkStringBuilderU &sb);
	...
	// returned.
	bool Eof(const uint16_t *handle);
	...
	// 
	int FileExists(const uint16_t *remotePath, bool followLinks);

	// Creates an asynchronous task to call the FileExists method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *FileExistsAsync(const uint16_t *remotePath, bool followLinks);
	...
	// 
	bool Fsync(const uint16_t *handle);

	// Creates an asynchronous task to call the Fsync method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *FsyncAsync(const uint16_t *handle);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkDateTimeU *GetFileCreateDt(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileCreateDt method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileCreateDtAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	// return a file's creation date/time.
	// 
	bool GetFileCreateTime(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, SYSTEMTIME &outSysTime);

	// The same as GetFileCreateTime, except the date/time is returned as an RFC822
	// formatted string.
	bool GetFileCreateTimeStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, CkString &outStr);
	// The same as GetFileCreateTime, except the date/time is returned as an RFC822
	// formatted string.
	const uint16_t *getFileCreateTimeStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	// The same as GetFileCreateTime, except the date/time is returned as an RFC822
	// formatted string.
	const uint16_t *fileCreateTimeStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileCreateTimeStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileCreateTimeStrAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	// 
	bool GetFileGroup(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, CkString &outStr);
	// Returns the group of a file. pathOrHandle may be a remote filepath or an open handle
	// string as returned by OpenFile. If pathOrHandle is a handle, then bIsHandle must be set to
	// true, otherwise it should be false. If bFollowLinks is true, then symbolic links
	// will be followed on the server.
	...
	// return a file's group name. Instead, the decimal GID of the file is returned.
	// 
	const uint16_t *getFileGroup(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	// Returns the group of a file. pathOrHandle may be a remote filepath or an open handle
	// string as returned by OpenFile. If pathOrHandle is a handle, then bIsHandle must be set to
	// true, otherwise it should be false. If bFollowLinks is true, then symbolic links
	// will be followed on the server.
	...
	// return a file's group name. Instead, the decimal GID of the file is returned.
	// 
	const uint16_t *fileGroup(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileGroup method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileGroupAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	// symbolic links will be followed on the server.
	bool GetFileLastAccess(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, SYSTEMTIME &outSysTime);
	...
	// symbolic links will be followed on the server.
	// The caller is responsible for deleting the object returned by this method.
	CkDateTimeU *GetFileLastAccessDt(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileLastAccessDt method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileLastAccessDtAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// The same as GetFileLastAccess, except the date/time is returned as an RFC822
	// formatted string.
	bool GetFileLastAccessStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, CkString &outStr);
	// The same as GetFileLastAccess, except the date/time is returned as an RFC822
	// formatted string.
	const uint16_t *getFileLastAccessStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	// The same as GetFileLastAccess, except the date/time is returned as an RFC822
	// formatted string.
	const uint16_t *fileLastAccessStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileLastAccessStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileLastAccessStrAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Returns the last-modified date/time for a file. pathOrHandle may be a remote filepath or
	// an open handle string as returned by OpenFile. If pathOrHandle is a handle, then bIsHandle
	// must be set to true, otherwise it should be false. If bFollowLinks is true, then
	// symbolic links will be followed on the server.
	bool GetFileLastModified(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, SYSTEMTIME &outSysTime);
	...
	// symbolic links will be followed on the server.
	// The caller is responsible for deleting the object returned by this method.
	CkDateTimeU *GetFileLastModifiedDt(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileLastModifiedDt method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileLastModifiedDtAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// The same as GetFileLastModified, except the date/time is returned as an RFC822
	// formatted string.
	bool GetFileLastModifiedStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, CkString &outStr);
	// The same as GetFileLastModified, except the date/time is returned as an RFC822
	// formatted string.
	const uint16_t *getFileLastModifiedStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	// The same as GetFileLastModified, except the date/time is returned as an RFC822
	// formatted string.
	const uint16_t *fileLastModifiedStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileLastModifiedStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileLastModifiedStrAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	// 
	bool GetFileOwner(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, CkString &outStr);
	// Returns the owner of a file. pathOrHandle may be a remote filepath or an open handle
	// string as returned by OpenFile. If pathOrHandle is a handle, then bIsHandle must be set to
	// true, otherwise it should be false. If bFollowLinks is true, then symbolic links
	// will be followed on the server.
	...
	// return a file's owner name. Instead, the decimal UID of the file is returned.
	// 
	const uint16_t *getFileOwner(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	// Returns the owner of a file. pathOrHandle may be a remote filepath or an open handle
	// string as returned by OpenFile. If pathOrHandle is a handle, then bIsHandle must be set to
	// true, otherwise it should be false. If bFollowLinks is true, then symbolic links
	// will be followed on the server.
	...
	// return a file's owner name. Instead, the decimal UID of the file is returned.
	// 
	const uint16_t *fileOwner(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFileOwner method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFileOwnerAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	// must be set to true, otherwise it should be false. If bFollowLinks is true, then
	// symbolic links will be followed on the server.
	int GetFilePermissions(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);

	// Creates an asynchronous task to call the GetFilePermissions method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *GetFilePermissionsAsync(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	int GetFileSize32(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	// the server.
	__int64 GetFileSize64(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	// 
	bool GetFileSizeStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle, CkString &outStr);
	// Returns the size in bytes (in decimal string form) of a file on the SSH server.
	// pathOrHandle may be a remote filepath or an open handle string as returned by OpenFile.
	// If pathOrHandle is a handle, then bIsHandle must be set to true, otherwise it should be
	// false. If bFollowLinks is true, then symbolic links will be followed on the server.
	...
	// such as ReadFileBytes64s allow for 64-bit values to be passed as strings.
	// 
	const uint16_t *getFileSizeStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	// Returns the size in bytes (in decimal string form) of a file on the SSH server.
	// pathOrHandle may be a remote filepath or an open handle string as returned by OpenFile.
	// If pathOrHandle is a handle, then bIsHandle must be set to true, otherwise it should be
	// false. If bFollowLinks is true, then symbolic links will be followed on the server.
	...
	// such as ReadFileBytes64s allow for 64-bit values to be passed as strings.
	// 
	const uint16_t *fileSizeStr(const uint16_t *pathOrHandle, bool bFollowLinks, bool bIsHandle);
	...
	bool HardLink(const uint16_t *oldPath, const uint16_t *newPath);

	// Creates an asynchronous task to call the HardLink method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *HardLinkAsync(const uint16_t *oldPath, const uint16_t *newPath);
	...
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *InitializeSftpAsync(void);
	...
	// returns false.
	bool LastReadFailed(const uint16_t *handle);

	// Returns the number of bytes received by the last read on a specified channel.
	int LastReadNumBytes(const uint16_t *handle);
	...
	// 
	bool OpenDir(const uint16_t *path, CkString &outStr);
	// Opens a directory for reading. To get a directory listing, first open the
	// directory by calling this method, then call ReadDir to read the directory, and
	// finally call CloseHandle to close the directory.
	// 
	...
	// directory of the SSH user account.
	// 
	const uint16_t *openDir(const uint16_t *path);

	// Creates an asynchronous task to call the OpenDir method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *OpenDirAsync(const uint16_t *path);
	...
	// 
	bool OpenFile(const uint16_t *remotePath, const uint16_t *access, const uint16_t *createDisposition, CkString &outStr);
	// Opens or creates a file on the remote system. Returns a handle which may be
	// passed to methods for reading and/or writing the file. The remotePath is the remote
	// file path (the path to the file on the server). When the application is finished
	// with the handle, it should call CloseHandle(remotePath).
	...
	// (the default value is true)
	// 
	const uint16_t *openFile(const uint16_t *remotePath, const uint16_t *access, const uint16_t *createDisposition);

	// Creates an asynchronous task to call the OpenFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *OpenFileAsync(const uint16_t *remotePath, const uint16_t *access, const uint16_t *createDisposition);
	...
	// The caller is responsible for deleting the object returned by this method.
	CkSFtpDirU *ReadDir(const uint16_t *handle);

	// Creates an asynchronous task to call the ReadDir method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ReadDirAsync(const uint16_t *handle);
	...
	// returns true.
	// 
	bool ReadFileBd(const uint16_t *handle, int numBytes, CkBinDataU &bd);

	// Creates an asynchronous task to call the ReadFileBd method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ReadFileBdAsync(const uint16_t *handle, int numBytes, CkBinDataU &bd);
	...
	// returns true.
	// 
	bool ReadFileBytes(const uint16_t *handle, int numBytes, CkByteData &outBytes);

	// Creates an asynchronous task to call the ReadFileBytes method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ReadFileBytesAsync(const uint16_t *handle, int numBytes);
	...
	bool ReadFileBytes32(const uint16_t *handle, int offset, int numBytes, CkByteData &outBytes);
	...
	// bytes, then fewer bytes may be returned.
	bool ReadFileBytes64(const uint16_t *handle, __int64 offset, int numBytes, CkByteData &outBytes);
	...
	// returned.
	// 
	bool ReadFileBytes64s(const uint16_t *handle, const uint16_t *offset, int numBytes, CkByteData &outBytes);
	...
	// returns true.
	// 
	bool ReadFileText(const uint16_t *handle, int numBytes, const uint16_t *charset, CkString &outStr);
	// This method is identical to ReadFileBytes except for one thing: The bytes are
	// interpreted according to the specified charset (i.e. the character encoding) and
	// returned as a string. A list of supported charset values may be found on this
	// page:Supported Charsets
	...
	// returns true.
	// 
	const uint16_t *readFileText(const uint16_t *handle, int numBytes, const uint16_t *charset);

	// Creates an asynchronous task to call the ReadFileText method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ReadFileTextAsync(const uint16_t *handle, int numBytes, const uint16_t *charset);
	...
	// all of the iso-8859-* encodings, or the Windows-* encodings.
	// 
	bool ReadFileText32(const uint16_t *handle, int offset, int numBytes, const uint16_t *charset, CkString &outStr);
	// This method is identical to ReadFileBytes32 except for one thing: The bytes are
	// interpreted according to the specified charset (i.e. the character encoding) and
	// returned as a string. A list of supported charset values may be found on this
	// page:Supported Charsets
	...
	// all of the iso-8859-* encodings, or the Windows-* encodings.
	// 
	const uint16_t *readFileText32(const uint16_t *handle, int offset, int numBytes, const uint16_t *charset);
	...
	// all of the iso-8859-* encodings, or the Windows-* encodings.
	// 
	bool ReadFileText64(const uint16_t *handle, __int64 offset, int numBytes, const uint16_t *charset, CkString &outStr);
	// This method is identical to ReadFileBytes64 except for one thing: The bytes are
	// interpreted according to the specified charset (i.e. the character encoding) and
	// returned as a string. A list of supported charset values may be found on this
	// page:Supported Charsets
	...
	// all of the iso-8859-* encodings, or the Windows-* encodings.
	// 
	const uint16_t *readFileText64(const uint16_t *handle, __int64 offset, int numBytes, const uint16_t *charset);
	...
	// all of the iso-8859-* encodings, or the Windows-* encodings.
	// 
	bool ReadFileText64s(const uint16_t *handle, const uint16_t *offset, int numBytes, const uint16_t *charset, CkString &outStr);
	// This method is identical to ReadFileBytes64s except for one thing: The bytes are
	// interpreted according to the specified charset (i.e. the character encoding) and
	// returned as a string. A list of supported charset values may be found on this
	// page:Supported Charsets
	...
	// all of the iso-8859-* encodings, or the Windows-* encodings.
	// 
	const uint16_t *readFileText64s(const uint16_t *handle, const uint16_t *offset, int numBytes, const uint16_t *charset);

	// Returns the target of a symbolic link on the server. The path is the path of the
	// symbolic link on the server.
	bool ReadLink(const uint16_t *path, CkString &outStr);
	// Returns the target of a symbolic link on the server. The path is the path of the
	// symbolic link on the server.
	const uint16_t *readLink(const uint16_t *path);

	// Creates an asynchronous task to call the ReadLink method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ReadLinkAsync(const uint16_t *path);
	...
	// 
	bool RealPath(const uint16_t *originalPath, const uint16_t *composePath, CkString &outStr);
	// This method can be used to have the server canonicalize any given path name to
	// an absolute path. This is useful for converting path names containing ".."
	// components or relative pathnames without a leading slash into absolute paths.
	// The absolute path is returned by this method.
	...
	// Note: Servers running SFTP v4 and below do not support composePath.
	// 
	const uint16_t *realPath(const uint16_t *originalPath, const uint16_t *composePath);

	// Creates an asynchronous task to call the RealPath method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *RealPathAsync(const uint16_t *originalPath, const uint16_t *composePath);
	...
	bool RemoveDir(const uint16_t *path);

	// Creates an asynchronous task to call the RemoveDir method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *RemoveDirAsync(const uint16_t *path);

	// Deletes a file on the SFTP server.
	bool RemoveFile(const uint16_t *filename);

	// Creates an asynchronous task to call the RemoveFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *RemoveFileAsync(const uint16_t *filename);
	...
	bool RenameFileOrDir(const uint16_t *oldPath, const uint16_t *newPath);

	// Creates an asynchronous task to call the RenameFileOrDir method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *RenameFileOrDirAsync(const uint16_t *oldPath, const uint16_t *newPath);
	...
	bool ResumeDownloadFileByName(const uint16_t *remoteFilePath, const uint16_t *localFilePath);
	...
	// v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ResumeDownloadFileByNameAsync(const uint16_t *remoteFilePath, const uint16_t *localFilePath);
	...
	bool ResumeUploadFileByName(const uint16_t *remoteFilePath, const uint16_t *localFilePath);

	// Creates an asynchronous task to call the ResumeUploadFileByName method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *ResumeUploadFileByNameAsync(const uint16_t *remoteFilePath, const uint16_t *localFilePath);
	...
	// 
	bool SetCreateDt(const uint16_t *pathOrHandle, bool isHandle, CkDateTimeU &createDateTime);

	// Creates an asynchronous task to call the SetCreateDt method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetCreateDtAsync(const uint16_t *pathOrHandle, bool isHandle, CkDateTimeU &createDateTime);
	...
	// setting the create date/time.
	// 
	bool SetCreateTime(const uint16_t *pathOrHandle, bool isHandle, SYSTEMTIME &createDateTime);

	// The same as SetCreateTime, except the date/time is passed as an RFC822 formatted
	// string.
	bool SetCreateTimeStr(const uint16_t *pathOrHandle, bool bIsHandle, const uint16_t *dateTimeStr);

	// Creates an asynchronous task to call the SetCreateTimeStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetCreateTimeStrAsync(const uint16_t *pathOrHandle, bool bIsHandle, const uint16_t *dateTimeStr);
	...
	bool SetLastAccessDt(const uint16_t *pathOrHandle, bool isHandle, CkDateTimeU &accessDateTime);

	// Creates an asynchronous task to call the SetLastAccessDt method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetLastAccessDtAsync(const uint16_t *pathOrHandle, bool isHandle, CkDateTimeU &accessDateTime);

	// Sets the last-access date/time for a file on the server. The pathOrHandle may be a
	// filepath or the handle of a currently open file. isHandle should be set to true if
	// the pathOrHandle is a handle, otherwise set isHandle to false.
	bool SetLastAccessTime(const uint16_t *pathOrHandle, bool isHandle, SYSTEMTIME &accessDateTime);

	// The same as SetLastAccessTime, except the date/time is passed as an RFC822
	// formatted string.
	bool SetLastAccessTimeStr(const uint16_t *pathOrHandle, bool bIsHandle, const uint16_t *dateTimeStr);

	// Creates an asynchronous task to call the SetLastAccessTimeStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetLastAccessTimeStrAsync(const uint16_t *pathOrHandle, bool bIsHandle, const uint16_t *dateTimeStr);

	// Sets the last-modified date/time for a file on the server. The pathOrHandle may be a
	// filepath or the handle of a currently open file. isHandle should be set to true if
	// the pathOrHandle is a handle, otherwise set isHandle to false.
	bool SetLastModifiedDt(const uint16_t *pathOrHandle, bool isHandle, CkDateTimeU &modifiedDateTime);

	// Creates an asynchronous task to call the SetLastModifiedDt method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetLastModifiedDtAsync(const uint16_t *pathOrHandle, bool isHandle, CkDateTimeU &modifiedDateTime);

	// Sets the last-modified date/time for a file on the server. The pathOrHandle may be a
	// filepath or the handle of a currently open file. isHandle should be set to true if
	// the pathOrHandle is a handle, otherwise set isHandle to false.
	bool SetLastModifiedTime(const uint16_t *pathOrHandle, bool isHandle, SYSTEMTIME &modifiedDateTime);

	// The same as SetLastModifiedTime, except the date/time is passed as an RFC822
	// formatted string.
	bool SetLastModifiedTimeStr(const uint16_t *pathOrHandle, bool bIsHandle, const uint16_t *dateTimeStr);

	// Creates an asynchronous task to call the SetLastModifiedTimeStr method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetLastModifiedTimeStrAsync(const uint16_t *pathOrHandle, bool bIsHandle, const uint16_t *dateTimeStr);
	...
	// 
	bool SetOwnerAndGroup(const uint16_t *pathOrHandle, bool isHandle, const uint16_t *owner, const uint16_t *group);

	// Creates an asynchronous task to call the SetOwnerAndGroup method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetOwnerAndGroupAsync(const uint16_t *pathOrHandle, bool isHandle, const uint16_t *owner, const uint16_t *group);
	...
	bool SetPermissions(const uint16_t *pathOrHandle, bool isHandle, int permissions);

	// Creates an asynchronous task to call the SetPermissions method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SetPermissionsAsync(const uint16_t *pathOrHandle, bool isHandle, int permissions);
	...
	bool SymLink(const uint16_t *oldPath, const uint16_t *newPath);

	// Creates an asynchronous task to call the SymLink method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SymLinkAsync(const uint16_t *oldPath, const uint16_t *newPath);
	...
	// descended.
	// 
	bool SyncTreeDownload(const uint16_t *remoteRoot, const uint16_t *localRoot, int mode, bool recurse);

	// Creates an asynchronous task to call the SyncTreeDownload method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *SyncTreeDownloadAsync(const uint16_t *remoteRoot, const uint16_t *localRoot, int mode, bool recurse);
	...
	bool UploadBd(CkBinDataU &binData, const uint16_t *remoteFilePath);

	// Creates an asynchronous task to call the UploadBd method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *UploadBdAsync(CkBinDataU &binData, const uint16_t *remoteFilePath);
	...
	bool UploadFile(const uint16_t *handle, const uint16_t *fromLocalFilePath);

	// Creates an asynchronous task to call the UploadFile method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *UploadFileAsync(const uint16_t *handle, const uint16_t *fromLocalFilePath);
	...
	// 
	bool UploadFileByName(const uint16_t *remoteFilePath, const uint16_t *localFilePath);

	// Creates an asynchronous task to call the UploadFileByName method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *UploadFileByNameAsync(const uint16_t *remoteFilePath, const uint16_t *localFilePath);

	// Uploads the contents of a StringBuilder to a remote file.
	bool UploadSb(CkStringBuilderU &sb, const uint16_t *remoteFilePath, const uint16_t *charset, bool includeBom);

	// Creates an asynchronous task to call the UploadSb method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *UploadSbAsync(CkStringBuilderU &sb, const uint16_t *remoteFilePath, const uint16_t *charset, bool includeBom);

	// Appends the contents of bd to an open file. The handle is a file handle returned
	// by the OpenFile method.
	bool WriteFileBd(const uint16_t *handle, CkBinDataU &bd);

	// Creates an asynchronous task to call the WriteFileBd method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *WriteFileBdAsync(const uint16_t *handle, CkBinDataU &bd);

	// Appends byte data to an open file. The handle is a file handle returned by the
	// OpenFile method.
	bool WriteFileBytes(const uint16_t *handle, CkByteData &byteData);

	// Creates an asynchronous task to call the WriteFileBytes method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *WriteFileBytesAsync(const uint16_t *handle, CkByteData &byteData);
	...
	bool WriteFileBytes32(const uint16_t *handle, int offset, CkByteData &data);

	// Writes data to an open file at a specific offset from the beginning of the file.
	// The handle is a file handle returned by the OpenFile method. The offset64 is an offset
	// from the beginning of the file.
	bool WriteFileBytes64(const uint16_t *handle, __int64 offset64, CkByteData &data);

	// Writes data to an open file at a specific offset from the beginning of the file.
	// The handle is a file handle returned by the OpenFile method. The offset64 is an offset
	// (in decimal string format) from the beginning of the file.
	bool WriteFileBytes64s(const uint16_t *handle, const uint16_t *offset64, CkByteData &data);
	...
	// 
	bool WriteFileText(const uint16_t *handle, const uint16_t *charset, const uint16_t *textData);

	// Creates an asynchronous task to call the WriteFileText method with the arguments
	// provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskU *WriteFileTextAsync(const uint16_t *handle, const uint16_t *charset, const uint16_t *textData);
	...
	// page:Supported Charsets
	// <http://www.chilkatsoft.com/p/p_463.asp>.
	bool WriteFileText32(const uint16_t *handle, int offset32, const uint16_t *charset, const uint16_t *textData);
	...
	// page:Supported Charsets
	// <http://www.chilkatsoft.com/p/p_463.asp>.
	bool WriteFileText64(const uint16_t *handle, __int64 offset64, const uint16_t *charset, const uint16_t *textData);
	...
	// page:Supported Charsets
	// <http://www.chilkatsoft.com/p/p_463.asp>.
	bool WriteFileText64s(const uint16_t *handle, const uint16_t *offset64, const uint16_t *charset, const uint16_t *textData);
