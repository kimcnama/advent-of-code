...
class CK_VISIBLE_PUBLIC CkLogU  : public CkUtf16Base
{
	...
	// Logs binary data in base64 format.
	void LogDataBase64_2(const uint16_t *tag, const void *pByteData, unsigned long szByteData);

	// Logs binary data in hex format.
	void LogDataHex(const uint16_t *tag, CkByteData &data);

	// Logs binary data in hex format.
	void LogDataHex2(const uint16_t *tag, const void *pByteData, unsigned long szByteData);
	...
	void LogHash2(const uint16_t *tag, const uint16_t *hashAlg, const void *pByteData, unsigned long szByteData);
