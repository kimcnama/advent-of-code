// CkXmlU.h: interface for the CkXmlU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkXmlU
class CK_VISIBLE_PUBLIC CkXmlU  : public CkUtf16Base
{
	...
	// Because the child nodes are not included, the content of "tag1" and "tag2" are
	// both equal to "This is the content".
	void put_Content(const uint16_t *newVal);
	...
	// The DOCTYPE declaration (if any) for the XML document.
	const uint16_t *docType(void);
	// The DOCTYPE declaration (if any) for the XML document.
	void put_DocType(const uint16_t *newVal);
	...
	// "utf-8".
	// 
	void put_Encoding(const uint16_t *newVal);
	...
	// The XML node's tag, including the namespace prefix.
	const uint16_t *tag(void);
	// The XML node's tag, including the namespace prefix.
	void put_Tag(const uint16_t *newVal);
	...
	// The node's namespace prefix, if present. For example, if the tag is
	// "soapenv:Envelope", then this property will return "soapenv".
	const uint16_t *tagNsPrefix(void);
	// The node's namespace prefix, if present. For example, if the tag is
	// "soapenv:Envelope", then this property will return "soapenv".
	void put_TagNsPrefix(const uint16_t *newVal);
	...
	// The node's tag without the namespace prefix. For example, if the tag is
	// "soapenv:Envelope", then this property will return "Envelope".
	const uint16_t *tagUnprefixed(void);
	// The node's tag without the namespace prefix. For example, if the tag is
	// "soapenv:Envelope", then this property will return "Envelope".
	void put_TagUnprefixed(const uint16_t *newVal);
	...
	// result.
	bool AccumulateTagContent(const uint16_t *tag, const uint16_t *skipTags, CkString &outStr);
	// Accumulates the content of all nodes having a specific tag into a single result
	// string. SkipTags specifies a set of subtrees to be avoided. The skipTags are
	// formatted as a string of tags delimited by vertical bar characters. All nodes in
	// sub-trees rooted with a tag appearing in skipTags are not included in the
	// result.
	const uint16_t *accumulateTagContent(const uint16_t *tag, const uint16_t *skipTags);

	// Adds an attribute to the calling node in the XML document. Returns True for
	// success, and False for failure.
	bool AddAttribute(const uint16_t *name, const uint16_t *value);

	// Adds an integer attribute to a node.
	bool AddAttributeInt(const uint16_t *name, int value);
	...
	void AddStyleSheet(const uint16_t *styleSheet);
	...
	bool BEncodeContent(const uint16_t *charset, CkByteData &inData);
	...
	// "tagA|tagB|tagC".
	// 
	bool ChildContentMatches(const uint16_t *tagPath, const uint16_t *pattern, bool caseSensitive);
	...
	// 
	bool ChilkatPath(const uint16_t *pathCmd, CkString &outStr);
	// Follows a series of commands to navigate through an XML document to return a
	// piece of data or update the caller's reference to a new XML document node.
	// 
	// Note: This method not related to the XPath (XML Path) standard in any way.
	...
	//     following the series of commands). Returns an empty string.
	// 
	const uint16_t *chilkatPath(const uint16_t *pathCmd);
	...
	bool ContentMatches(const uint16_t *pattern, bool caseSensitive);

	// Copies the tag, content, and attributes to the calling node.
	void Copy(CkXmlU &node);
	...
	void CopyRef(CkXmlU &copyFromNode);
	...
	bool DecodeEntities(const uint16_t *str, CkString &outStr);
	// Utility method to decode HTML entities. It accepts a string containing
	// (potentially) HTML entities and returns a string with the entities decoded.
	const uint16_t *decodeEntities(const uint16_t *str);

	// Decrypts the content of an XML node that was previously 128-bit AES encrypted
	// with the EncryptContent method.
	bool DecryptContent(const uint16_t *password);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *ExtractChildByName(const uint16_t *tagPath, const uint16_t *attrName, const uint16_t *attrValue);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *FindChild(const uint16_t *tagPath);
	...
	// "tagA|tagB|tagC".
	// 
	bool FindChild2(const uint16_t *tagPath);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *FindNextRecord(const uint16_t *tagPath, const uint16_t *contentPattern);
	...
	// Returns the name of the Nth attribute of an XML node. The first attribute is at
	// index 0.
	const uint16_t *getAttributeName(int index);
	// Returns the name of the Nth attribute of an XML node. The first attribute is at
	// index 0.
	const uint16_t *attributeName(int index);
	...
	// Returns the value of the Nth attribute of an XML node. The first attribute is at
	// index 0.
	const uint16_t *getAttributeValue(int index);
	// Returns the value of the Nth attribute of an XML node. The first attribute is at
	// index 0.
	const uint16_t *attributeValue(int index);
	...
	bool GetAttrValue(const uint16_t *name, CkString &outStr);
	// Find and return the value of an attribute having a specified name.
	const uint16_t *getAttrValue(const uint16_t *name);
	// Find and return the value of an attribute having a specified name.
	const uint16_t *attrValue(const uint16_t *name);

	// Returns an attribute as an integer. Returns 0 if the attribute does not exist.
	int GetAttrValueInt(const uint16_t *name);
	...
	bool GetBinaryContent(bool unzipFlag, bool decryptFlag, const uint16_t *password, CkByteData &outData);

	// Returns the Nth child of an XML node
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *GetChild(int index);
	...
	bool GetChildAttrValue(const uint16_t *tagPath, const uint16_t *attrName, CkString &outStr);
	// Returns the content of a descendant child having a specified attribute. The tagPath
	// can be a tag or a tag path.
	const uint16_t *getChildAttrValue(const uint16_t *tagPath, const uint16_t *attrName);
	// Returns the content of a descendant child having a specified attribute. The tagPath
	// can be a tag or a tag path.
	const uint16_t *childAttrValue(const uint16_t *tagPath, const uint16_t *attrName);
	...
	// "colors|primary|red".
	// 
	bool GetChildBoolValue(const uint16_t *tagPath);
	...
	// "colors|primary|red".
	// 
	bool GetChildContent(const uint16_t *tagPath, CkString &outStr);
	// Returns the content of a child having a specified tag. The tagPath can be a tag or
	// a tag path.
	// 
	// Beginning in version 9.5.0.64, the tagPath can be a tag path. A tag path is a
	...
	// "colors|primary|red".
	// 
	const uint16_t *getChildContent(const uint16_t *tagPath);
	// Returns the content of a child having a specified tag. The tagPath can be a tag or
	// a tag path.
	// 
	// Beginning in version 9.5.0.64, the tagPath can be a tag path. A tag path is a
	...
	// "colors|primary|red".
	// 
	const uint16_t *childContent(const uint16_t *tagPath);

	// Returns the content of the Nth child node.
	bool GetChildContentByIndex(int index, CkString &outStr);
	// Returns the content of the Nth child node.
	const uint16_t *getChildContentByIndex(int index);
	// Returns the content of the Nth child node.
	const uint16_t *childContentByIndex(int index);

	// Returns the content of the XML element at the tagPath. The XML element's content is
	// appended to the sb.
	bool GetChildContentSb(const uint16_t *tagPath, CkStringBuilderU &sb);

	// Returns the child having the exact tag and content.
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *GetChildExact(const uint16_t *tag, const uint16_t *content);
	...
	// "colors|primary|red".
	// 
	int GetChildIntValue(const uint16_t *tagPath);

	// Returns the tag name of the Nth child node.
	bool GetChildTag(int index, CkString &outStr);
	// Returns the tag name of the Nth child node.
	const uint16_t *getChildTag(int index);
	// Returns the tag name of the Nth child node.
	const uint16_t *childTag(int index);

	// Returns the tag name of the Nth child node.
	bool GetChildTagByIndex(int index, CkString &outStr);
	// Returns the tag name of the Nth child node.
	const uint16_t *getChildTagByIndex(int index);
	// Returns the tag name of the Nth child node.
	const uint16_t *childTagByIndex(int index);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *GetChildWithAttr(const uint16_t *tagPath, const uint16_t *attrName, const uint16_t *attrValue);

	// Returns the first child found having the exact content specified.
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *GetChildWithContent(const uint16_t *content);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *GetChildWithTag(const uint16_t *tagPath);
	...
	// "tagA|tagB|tagC".
	// 
	bool HasChildWithTag(const uint16_t *tagPath);
	...
	// "tagA|tagB|tagC".
	// 
	bool HasChildWithTagAndContent(const uint16_t *tagPath, const uint16_t *content);

	// Adds an entire subtree as a child. If the child was a subtree within another Xml
	// document then the subtree is effectively transferred from one XML document to
	// another. The child tree is inserted in a position after the Nth child (of the
	// calling node).
	void InsertChildTreeAfter(int index, CkXmlU &tree);

	// Adds an entire subtree as a child. If the child was a subtree within another Xml
	// document then the subtree is effectively transferred from one XML document to
	// another. The child tree is inserted in a position before the Nth child (of the
	// calling node).
	void InsertChildTreeBefore(int index, CkXmlU &tree);
	...
	bool LoadBd(CkBinDataU &bd, bool autoTrim);

	// Loads XML from the contents of a StringBuilder object.
	bool LoadSb(CkStringBuilderU &sb, bool autoTrim);
	...
	// loaded.
	bool LoadXml(const uint16_t *xmlData);
	...
	bool LoadXml2(const uint16_t *xmlData, bool autoTrim);
	...
	bool LoadXmlFile(const uint16_t *fileName);

	// Same as LoadXmlFile, but an additional argument controls whether or not
	// leading/trailing whitespace is auto-trimmed from each leaf node's content.
	bool LoadXmlFile2(const uint16_t *fileName, bool autoTrim);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *NewChild(const uint16_t *tagPath, const uint16_t *content);
	...
	// "colors|primary|red". See the example below for details.
	// 
	void NewChild2(const uint16_t *tagPath, const uint16_t *content);

	// Inserts a new child in a position after the Nth child node.
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *NewChildAfter(int index, const uint16_t *tag, const uint16_t *content);

	// Inserts a new child in a position before the Nth child node.
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *NewChildBefore(int index, const uint16_t *tag, const uint16_t *content);
	...
	// "colors|primary|red". See the example below for details.
	// 
	void NewChildInt2(const uint16_t *tagPath, int value);
	...
	// 
	bool NextInTraversal2(CkStringBuilderU &sbState);

	// Returns the nodes next sibling, or NULL if there are no more.
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *NextSibling(void);
	...
	bool QEncodeContent(const uint16_t *charset, CkByteData &inData);
	...
	bool RemoveAttribute(const uint16_t *name);
	...
	// "tagA|tagB|tagC".
	// 
	void RemoveChild(const uint16_t *tagPath);
	...
	void RemoveChildWithContent(const uint16_t *content);
	...
	int RemoveStyleSheet(const uint16_t *attrName, const uint16_t *attrValue);

	// Saves a node's binary content to a file.
	bool SaveBinaryContent(const uint16_t *filename, bool unzipFlag, bool decryptFlag, const uint16_t *password);
	...
	// will find it useful.
	// 
	void Scrub(const uint16_t *directives);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *SearchAllForContent(CkXmlU *afterPtr, const uint16_t *contentPattern);
	...
	bool SearchAllForContent2(CkXmlU *afterPtr, const uint16_t *contentPattern);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *SearchForAttribute(CkXmlU *afterPtr, const uint16_t *tag, const uint16_t *attr, const uint16_t *valuePattern);

	// Same as SearchForAttribute except the internal reference of the caller is
	// updated to point to the search result (instead of returning a new object).
	bool SearchForAttribute2(CkXmlU *afterPtr, const uint16_t *tag, const uint16_t *attr, const uint16_t *valuePattern);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *SearchForContent(CkXmlU *afterPtr, const uint16_t *tag, const uint16_t *contentPattern);
	...
	bool SearchForContent2(CkXmlU *afterPtr, const uint16_t *tag, const uint16_t *contentPattern);
	...
	// 
	// The caller is responsible for deleting the object returned by this method.
	CkXmlU *SearchForTag(CkXmlU *afterPtr, const uint16_t *tag);
	...
	bool SearchForTag2(CkXmlU *afterPtr, const uint16_t *tag);
	...
	bool SetBinaryContent(CkByteData &inData, bool zipFlag, bool encryptFlag, const uint16_t *password);

#if !defined(CHILKAT_MONO)
	// The same as SetBinaryContent but the data is provided via a pointer and byte
	// count.
	bool SetBinaryContent2(const void *pByteData, unsigned long szByteData, bool zipFlag, bool encryptFlag, const uint16_t *password);
#endif
	...
	// encoded.
	bool SetBinaryContentFromFile(const uint16_t *filename, bool zipFlag, bool encryptFlag, const uint16_t *password);

	// Sorts the direct child nodes by the value of a specified attribute.
	void SortByAttribute(const uint16_t *attrName, bool ascending);
	...
	void SortByAttributeInt(const uint16_t *attrName, bool ascending);
	...
	// nodes.
	void SortRecordsByAttribute(const uint16_t *sortTag, const uint16_t *attrName, bool ascending);

	// Sorts the direct child nodes by the content of the grandchild nodes.
	void SortRecordsByContent(const uint16_t *sortTag, bool ascending);
	...
	bool TagContent(const uint16_t *tagName, CkString &outStr);
	// Returns the content of the 1st node found in the sub-tree rooted at the caller
	// that has a given tag. (Note: The search for the node having tag ARG is not
	// limited to the direct children of the caller.)
	const uint16_t *tagContent(const uint16_t *tagName);

	// Returns true if the node's tag equals the specified string.
	bool TagEquals(const uint16_t *tag);
	...
	bool TagUnpEquals(const uint16_t *unprefixedTag);
	...
	bool UpdateAt(const uint16_t *tagPath, bool autoCreate, const uint16_t *value);
	...
	bool UpdateAttrAt(const uint16_t *tagPath, bool autoCreate, const uint16_t *attrName, const uint16_t *attrValue);

	// Adds an attribute to the node if it doesn't already exist. Otherwise it updates
	// the existing attribute with the new value.
	bool UpdateAttribute(const uint16_t *attrName, const uint16_t *attrValue);

	// Updates an attribute value. (Call UpdateAttribute if the attribute value is a
	// string.)
	bool UpdateAttributeInt(const uint16_t *attrName, int value);
	...
	// 
	void UpdateChildContent(const uint16_t *tagPath, const uint16_t *value);
	...
	// "tagA|tagB|tagC".
	// 
	void UpdateChildContentInt(const uint16_t *tagPath, int value);
