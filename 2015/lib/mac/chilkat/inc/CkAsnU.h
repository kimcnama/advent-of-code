// CkAsnU.h: interface for the CkAsnU class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkAsnU
class CK_VISIBLE_PUBLIC CkAsnU  : public CkUtf16Base
{
	...
	// BmpString, PrintableString, VisibleString, T61String, IA5String, NumericString,
	// or UniversalString).
	const uint16_t *contentStr(void);
	// The ASN.1 item's content if it is an ASN.1 string type (such as Utf8String,
	// BmpString, PrintableString, VisibleString, T61String, IA5String, NumericString,
	// or UniversalString).
	void put_ContentStr(const uint16_t *newVal);
	...
	bool AppendBigInt(const uint16_t *encodedBytes, const uint16_t *encoding);
	...
	bool AppendBits(const uint16_t *encodedBytes, const uint16_t *encoding);
	...
	bool AppendContextPrimitive(int tag, const uint16_t *encodedBytes, const uint16_t *encoding);
	...
	// "url_rfc1738", "url_rfc2396", and "url_rfc3986". The encoding name is case
	// insensitive (for example, both "Base64" and "base64" are treated the same).
	bool AppendOctets(const uint16_t *encodedBytes, const uint16_t *encoding);
	...
	bool AppendOid(const uint16_t *oid);
	...
	bool AppendString(const uint16_t *strType, const uint16_t *value);
	...
	// "YYMMDDhhmm[ss](+|-)hhmm".
	bool AppendTime(const uint16_t *timeFormat, const uint16_t *dateTimeStr);

	// Converts ASN.1 to XML and returns the XML string.
	bool AsnToXml(CkString &outStr);
	// Converts ASN.1 to XML and returns the XML string.
	const uint16_t *asnToXml(void);
	...
	// "url_oath", "url_rfc1738", "url_rfc2396", and "url_rfc3986". The encoding name is
	// case insensitive (for example, both "Base64" and "base64" are treated the same).
	bool GetEncodedContent(const uint16_t *encoding, CkString &outStr);
	// Returns the content of the ASN.1 item in encoded string form. The encoding may be
	// any of the following encodings: "Base64", "Hex", "Base58", "modBase64",
	// "Base32", "UU", "QP" (for quoted-printable), "URL" (for url-encoding), "Q", "B",
	// "url_oath", "url_rfc1738", "url_rfc2396", and "url_rfc3986". The encoding name is
	// case insensitive (for example, both "Base64" and "base64" are treated the same).
	const uint16_t *getEncodedContent(const uint16_t *encoding);
	// Returns the content of the ASN.1 item in encoded string form. The encoding may be
	// any of the following encodings: "Base64", "Hex", "Base58", "modBase64",
	// "Base32", "UU", "QP" (for quoted-printable), "URL" (for url-encoding), "Q", "B",
	// "url_oath", "url_rfc1738", "url_rfc2396", and "url_rfc3986". The encoding name is
	// case insensitive (for example, both "Base64" and "base64" are treated the same).
	const uint16_t *encodedContent(const uint16_t *encoding);
	...
	bool LoadBinaryFile(const uint16_t *path);

	// Loads ASN.1 from an encoded string. The encoding can be "base64", "hex", "uu",
	// "quoted-printable", "base32", or "modbase64".
	bool LoadEncoded(const uint16_t *asnContent, const uint16_t *encoding);
	...
	// "url_rfc1738", "url_rfc2396", and "url_rfc3986". The encoding name is case
	// insensitive (for example, both "Base64" and "base64" are treated the same).
	bool SetEncodedContent(const uint16_t *encodedBytes, const uint16_t *encoding);

	// Appends the ASN.1 in binary DER format to bd.
	bool WriteBd(CkBinDataU &bd);

	// Writes the ASN.1 in binary DER form to a file.
	bool WriteBinaryDer(const uint16_t *path);
