// CkHttpRequestU.h: interface for the CkHttpRequestU class.
//
//////////////////////////////////////////////////////////////////////

// This header is generated for Chilkat 9.5.0.89

#ifndef _CkHttpRequestU_H
#define _CkHttpRequestU_H
...
class CK_VISIBLE_PUBLIC CkHttpRequestU  : public CkUtf16Base
{
	...
	CkHttpRequestU(const CkHttpRequestU &);
	CkHttpRequestU &operator=(const CkHttpRequestU &);

    public:
	CkHttpRequestU(void);
	virtual ~CkHttpRequestU(void);

	

	static CkHttpRequestU *createNew(void);
	...
	// Composes and returns the entire MIME header of the HTTP request.
	const uint16_t *entireHeader(void);
	// Composes and returns the entire MIME header of the HTTP request.
	void put_EntireHeader(const uint16_t *newVal);
	...
	// MKCOL, COPY, MOVE, LOCK, UNLOCK, etc. In general, the HttpVerb may be set to
	// anything, even custom verbs recognized by a custom server-side app.
	const uint16_t *httpVerb(void);
	// The HttpVerb property should be set to the name of the HTTP method that appears
	// on the "start line" of an HTTP request, such as GET, POST, PUT, DELETE, etc. It
	// is also possible to use the various WebDav verbs such as PROPFIND, PROPPATCH,
	// MKCOL, COPY, MOVE, LOCK, UNLOCK, etc. In general, the HttpVerb may be set to
	// anything, even custom verbs recognized by a custom server-side app.
	void put_HttpVerb(const uint16_t *newVal);

	// The HTTP version in the request header. Defaults to "1.1".
	void get_HttpVersion(CkString &str);
	// The HTTP version in the request header. Defaults to "1.1".
	const uint16_t *httpVersion(void);
	// The HTTP version in the request header. Defaults to "1.1".
	void put_HttpVersion(const uint16_t *newVal);
	...
	// (assuming the request returns HTML, otherwise a fragment makes no sense).
	// 
	void put_Path(const uint16_t *newVal);
	...
	// filename extension.
	// 
	bool AddBdForUpload(const uint16_t *name, const uint16_t *remoteFilename, CkBinDataU &byteData, const uint16_t *contentType);
	...
	// 
	bool AddBytesForUpload(const uint16_t *name, const uint16_t *remoteFileName, CkByteData &byteData);
	...
	bool AddBytesForUpload2(const uint16_t *name, const uint16_t *remoteFileName, CkByteData &byteData, const uint16_t *contentType);
	...
	// 
	bool AddFileForUpload(const uint16_t *name, const uint16_t *filePath);
	...
	// filePath is the path to an existing file in the local filesystem.
	// 
	bool AddFileForUpload2(const uint16_t *name, const uint16_t *filePath, const uint16_t *contentType);
	...
	// 
	void AddHeader(const uint16_t *name, const uint16_t *value);
	...
	// 
	bool AddMwsSignature(const uint16_t *domain, const uint16_t *mwsSecretKey);
	...
	void AddParam(const uint16_t *name, const uint16_t *value);

	// Same as AddFileForUpload, but the upload data comes from an in-memory string
	// instead of a file.
	bool AddStringForUpload(const uint16_t *name, const uint16_t *filename, const uint16_t *strData, const uint16_t *charset);
	...
	bool AddStringForUpload2(const uint16_t *name, const uint16_t *filename, const uint16_t *strData, const uint16_t *charset, const uint16_t *contentType);
	...
	bool AddSubHeader(int index, const uint16_t *name, const uint16_t *value);

	// The same as GenerateRequestText, except the generated request is written to the
	// file specified by path.
	bool GenerateRequestFile(const uint16_t *path);
	...
	// Returns the request text that would be sent if Http.SynchronousRequest was
	// called.
	const uint16_t *generateRequestText(void);

	// Returns the value of a request header field.
	bool GetHeaderField(const uint16_t *name, CkString &outStr);
	// Returns the value of a request header field.
	const uint16_t *getHeaderField(const uint16_t *name);
	// Returns the value of a request header field.
	const uint16_t *headerField(const uint16_t *name);
	...
	bool GetParam(const uint16_t *name, CkString &outStr);
	// Returns a request query parameter value by name.
	const uint16_t *getParam(const uint16_t *name);
	// Returns a request query parameter value by name.
	const uint16_t *param(const uint16_t *name);
	...
	// param1="abc 123" and param2="abc-123", then GetUrlEncodedParams would return
	// "abc+123
	const uint16_t *getUrlEncodedParams(void);
	// Returns the request parameters in URL encoded form (i.e. in the exact form that
	// would be sent if the ContentType property was
	// application/x-www-form-urlencoded). For example, if a request has two params:
	// param1="abc 123" and param2="abc-123", then GetUrlEncodedParams would return
	// "abc+123
	const uint16_t *urlEncodedParams(void);

	// Uses the contents of the requestBody as the HTTP request body.
	bool LoadBodyFromBd(CkBinDataU &requestBody);
	...
	// Note: A non-multipart HTTP request consists of (1) the HTTP start line, (2) MIME
	// header fields, and (3) the MIME body. This method sets the MIME body.
	bool LoadBodyFromFile(const uint16_t *filePath);
	...
	// are valid.
	bool LoadBodyFromSb(CkStringBuilderU &requestBody, const uint16_t *charset);
	...
	// 
	bool LoadBodyFromString(const uint16_t *bodyStr, const uint16_t *charset);
	...
	bool StreamBodyFromFile(const uint16_t *filePath);
	...
	// strings.
	bool StreamChunkFromFile(const uint16_t *path, const uint16_t *offset, const uint16_t *numBytes);
	...
	// 
	void UseXmlHttp(const uint16_t *xmlBody);
